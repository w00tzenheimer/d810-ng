# Chernobog: Pattern Engine Acceleration Project
...
Chernobog is a 5-PR performance optimization project targeting the pattern matching hot path in d810's microcode optimizer. The project maintains full backward compatibility through feature flags while delivering significant performance improvements.

### PR Sequence

| PR | Focus | Status | Key Change |
|----|-------|--------|------------|
| PR0 | Baseline Benchmarks | ✅ Complete | Established performance metrics |
| PR1 | Engine Dispatcher | ✅ Complete | Cython/Python backend gate |
| PR2 | Opcode-Indexed Storage | ✅ Complete | O(1) pattern lookup by opcode |
| PR3 | Registry Cache | ✅ Complete | Function-scoped rule filtering |
| PR4 | Non-Mutating Match | ✅ Complete | Eliminated clone overhead |
| PR5 | Validation & Policy | 🔄 This PR | Cython-first default policy |
...
### Full Revert to Pre-Chernobog Behavior
...
See `docs/chernobog/benchmarks/baseline_pattern_engine.json` for pre-optimization baseline numbers (generated by runtime tests).
...
2. Compare with `docs/chernobog/benchmarks/baseline_pattern_engine.json` (if available)
3. Check active backend with `get_engine_info()` — ensure Cython is actually enabled
