...
class TestAnalysisPipelineCoverage:
    ...
    def test_hints_produced(self, analysis_store_session):
        ...
        assert count >= 1, f"Expected >= 1 function with hints, got {count}"
    ...
    def test_session_summaries_match_hints(self, analysis_store_session):
        ...
        assert len(gap) == 0, (
            f"Functions with hints but no session summary: {[hex(ea) for ea in gap]}"
        )
    ...
    def test_collectors_fired_nonzero(self, analysis_store_session):
        ...
        zero_collector_funcs = [
            hex(s["func_ea"]) for s in summaries if s["collectors_fired"] == 0
        ]
