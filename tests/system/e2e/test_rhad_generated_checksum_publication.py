...
def _run_worker(binary: pathlib.Path) -> None:
    ...
    try:
        ...
        receipt = matching[0]
        assert receipt.operation_count == receipt.planned_operation_count == 929, (
...
def test_a560_generated_checksum_commits_and_reaches_ctree(
    ...
    with sqlite3.connect(diag_path) as connection:
        ...
        assert compiled_payload["aggregate_program_identity"] == (
            "sha256:e00f9e7aa8687891c1fb0fa202791ba23ffcf67e180b470d65d8d615f221603c"
        )
        proof_artifacts = {
        ...
        for maturity in ("MMAT_GENERATED", "MMAT_PREOPTIMIZED", "MMAT_LOCOPT"):
            ...
            assert row139["source_topology_reachable"] is True
            assert row139["source_topology_retired"] is False
            assert row139["indirect_transfer_present"] is False
            assert row139["target_eas"] == [0x40BB8F, 0x40BF8C]
        ...
        assert row134_calls["source_topology_reachable"] is True
        assert row134_calls["source_topology_retired"] is False
        assert row134_calls["indirect_transfer_present"] is False
        assert row134_calls["target_eas"] == [0x40B958, 0x40BCCB]
        ...
        assert {0x40BCCB, 0x40BCD9, 0x40BCDF, 0x40BCE3}.issubset(
            calls_payload["reachable_eas"]
        )
        assert {
            0x40B802,
