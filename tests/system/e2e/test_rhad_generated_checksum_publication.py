...
def _run_worker(binary: pathlib.Path) -> None:
    ...
    try:
        ...
        receipt = matching[0]
        assert receipt.operation_count == receipt.planned_operation_count == 817, (
...
def test_a560_generated_checksum_commits_and_reaches_ctree(
    ...
    with sqlite3.connect(diag_path) as connection:
        ...
        assert compiled_payload["aggregate_program_identity"] == (
            "sha256:3888b3f3501598188ba02fa2ea8edf516b2d1bf757400857aa42007f4f715d96"
        )
        proof_artifacts = {
        ...
        assert (
            row126_reference["proof_artifact"]
            == proof_artifacts["rhad:route@0x40B7F4"]
        )
        ...
        for maturity in ("MMAT_GENERATED", "MMAT_PREOPTIMIZED", "MMAT_LOCOPT"):
            ...
            assert row117["target_eas"] == [
                0x40B6F0,
                0x40B88E if maturity == "MMAT_LOCOPT" else 0x40B882,
            ]
            ...
            assert row127["source_topology_reachable"] is (
                maturity != "MMAT_LOCOPT"
            )
            assert row127["source_topology_retired"] is (
                maturity == "MMAT_LOCOPT"
            )
            ...
            assert row128["source_topology_reachable"] is (
                maturity != "MMAT_LOCOPT"
            )
            assert row128["source_topology_retired"] is (
                maturity == "MMAT_LOCOPT"
            )
        ...
        assert {0x40B7E6, 0x40BE2F}.issubset(
        ...
        assert {0x40B7E6, 0x40B802, 0x40B839, 0x40C150}.issubset(
            calls_payload["reachable_eas"]
        )
        assert {0x40A5F6, 0x40B760, 0x40C696}.issubset(
            calls_payload["reachable_eas"]
