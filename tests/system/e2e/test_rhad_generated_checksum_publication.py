...
def _run_worker(binary: pathlib.Path) -> None:
    ...
    try:
        ...
        receipt = matching[0]
        assert receipt.operation_count == receipt.planned_operation_count == 723, (
        ...
        assert 0x40B6F0 in captures["calls"]["reachable_eas"], captures["calls"]
        headless.stop()
...
def test_a560_generated_checksum_commits_and_reaches_ctree(
    ...
    with sqlite3.connect(diag_path) as connection:
        ...
        assert compiled_payload["aggregate_program_identity"] == (
            "sha256:6ac5b827b92e278ebda29fd83c08edc9461a0adb5a5fbb2829ea37c6a7ba914d"
        )
        proof_artifacts = {
        ...
        for maturity in ("MMAT_GENERATED", "MMAT_PREOPTIMIZED", "MMAT_LOCOPT"):
            ...
            assert row117["source_topology_reachable"] is True
            assert row117["source_topology_retired"] is False
            assert row117["indirect_transfer_present"] is False
            assert row117["target_eas"] == [0x40B6F0, 0x40B880]
        ...
        assert 0x40B6F0 in calls_payload["reachable_eas"]
