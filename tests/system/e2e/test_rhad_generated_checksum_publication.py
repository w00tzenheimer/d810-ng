...
def _run_worker(binary: pathlib.Path) -> None:
    ...
    try:
        ...
        receipt = matching[0]
        assert receipt.operation_count == receipt.planned_operation_count == 1100, (
...
def test_a560_generated_checksum_commits_and_reaches_ctree(
    ...
    with sqlite3.connect(diag_path) as connection:
        ...
        assert compiled_payload["aggregate_program_identity"] == (
            "sha256:6fb9f52f4db637844f8a31c94b9d2a717cc8148b1fac746152e79b28fae11899"
        )
        proof_artifacts = {
        ...
        for maturity in ("MMAT_GENERATED", "MMAT_PREOPTIMIZED", "MMAT_LOCOPT"):
            ...
            assert row139["target_eas"] == [
                0x40BB8F,
                0x40BF9A if maturity == "MMAT_LOCOPT" else 0x40BF8E,
            ]
        ...
        assert {0x40BEE7, 0x40BEF5, 0x40BEFB, 0x40BEFF}.issubset(
