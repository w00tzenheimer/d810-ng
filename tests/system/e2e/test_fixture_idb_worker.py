...
@pytest.mark.skipif(not IDB or not Path(IDB).exists(),
                    reason="set DAC_IDB to the dac.dll .i64 to run")
def test_extract_and_resolve_rand(tmp_path):
    out = tmp_path / "sub_1815C8C30.asm"
    env = {**os.environ, "PYTHONPATH": str(REPO / "src")}
    r = subprocess.run(
        [sys.executable, str(WORKER), "extract", "--idb", IDB,
         "--func", "sub_1815C8C30", "--out", str(out)],
    ...
    r2 = subprocess.run(
        [sys.executable, str(WORKER), "resolve", "--idb", IDB,
         "--vas", "0x181803620"],
        capture_output=True, text=True, env=env)
