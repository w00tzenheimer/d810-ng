...
def test_started_activation_replaces_live_rule_collections_for_stage_switches(
    ...
    with d810_state() as state:
        ...
        try:
            ...
            state._activate_project(project_index=9000, project=enabled)
...
def test_started_activation_drift_preserves_previous_state_and_scope(
    ...
    with d810_state() as state:
        ...
        state._activate_project(project_index=8999, project=baseline)
        _prepare_started_manager(state)
...
@pytest.mark.ida_required
def test_activation_drift_test_double_does_not_escape_the_test_registry() -> None:
    """A local drift model must not become a production rule candidate."""
    escaped = [
        rule_type
        for rule_type in InstructionOptimizationRule.registry.values()
        if rule_type.__name__ == "DriftedFoldReadonlyDataRule"
    ]
    assert escaped == []
...
def test_started_activation_restores_scope_caches_and_adapter_context_after_late_failure(
    ...
    with d810_state() as state:
        ...
        state._activate_project(project_index=8998, project=baseline)
        _prepare_started_manager(state)
        manager = state.manager
