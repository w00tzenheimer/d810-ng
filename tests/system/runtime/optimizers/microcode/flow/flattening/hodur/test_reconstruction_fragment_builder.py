...
class TestIntraFragmentDupAndRedirectConflicts:
    """Intra-fragment conflict detection between RedirectGoto/ConvertToGoto
    and DuplicateAndRedirect on the same source block.

    The tracer (``D810_TRACE_MOD_CONSTRUCTION=1``) revealed that on
    sub_7FFD3338C040 13 blocks have a DuplicateAndRedirect from
    ``plan_shared_group_reconstruction_modifications`` AND a RedirectGoto
    from another planner targeting the same source.  7+ have disagreeing
    targets (e.g. blk[100]: RedirectGoto->21 vs DupAndRedirect
    [(99,2),(98,21)] — pred 99 disagrees).

    Resolution policy: DuplicateAndRedirect wins.  Per-pred routing is
    strictly more expressive than uniform RedirectGoto on a shared block;
    the planner that emitted DupAndRedirect knew about the multi-pred
    nature, the one that emitted plain RedirectGoto did not.

    The cumulative_planner_view is *not* required for this filter — it
    runs on the current fragment's modification list directly.
    """

    def _finalize(self, modifications: list, owned_blocks: set[int] | None = None):
        return finalize_reconstruction_fragment(
            strategy_name="state_write_reconstruction",
    ...
    def test_drops_redirect_when_dup_disagrees_on_any_pred(self) -> None:
        # Tracer-observed pattern: blk[100] RedirectGoto->21 vs
        # DupAndRedirect [(99,2),(98,21)].  Pred 99 disagrees (RedirectGoto
        # would force 99 -> 21, but DupAndRedirect routes 99 -> 2).
        # DupAndRedirect wins.
        mods = [
            RedirectGoto(from_serial=100, old_target=2, new_target=21),
            DuplicateAndRedirect(
                source_serial=100, per_pred_targets=((99, 2), (98, 21)),
            ),
        ]
        frag = self._finalize(mods, owned_blocks={100})
        kinds = sorted(type(m).__name__ for m in frag.modifications)
        assert kinds == ["DuplicateAndRedirect"]

    def test_drops_redirect_when_dup_targets_all_match_redirect(self) -> None:
        # When all per_pred_targets agree with the RedirectGoto's
        # new_target, the RedirectGoto is redundant.  DupAndRedirect still
        # wins (more expressive — keeps the per-pred topology decision).
        mods = [
            RedirectGoto(from_serial=10, old_target=2, new_target=216),
            DuplicateAndRedirect(
                source_serial=10, per_pred_targets=((9, 216), (144, 216)),
            ),
        ]
        frag = self._finalize(mods, owned_blocks={10})
        kinds = sorted(type(m).__name__ for m in frag.modifications)
        assert kinds == ["DuplicateAndRedirect"]

    def test_drops_convert_to_goto_when_dup_present(self) -> None:
        # ConvertToGoto follows the same emission path as RedirectGoto and
        # is treated identically by the existing Mode 1 filter — extend
        # the same handling for DupAndRedirect overlap.
        mods = [
            ConvertToGoto(block_serial=42, goto_target=99),
            DuplicateAndRedirect(
                source_serial=42, per_pred_targets=((40, 50), (41, 99)),
            ),
        ]
        frag = self._finalize(mods, owned_blocks={42})
        kinds = sorted(type(m).__name__ for m in frag.modifications)
        assert kinds == ["DuplicateAndRedirect"]

    def test_keeps_redirect_when_dup_targets_different_source(self) -> None:
        # No conflict: RedirectGoto src=100, DupAndRedirect source_serial=200.
        # Different blocks; both kept.
        mods = [
            RedirectGoto(from_serial=100, old_target=2, new_target=21),
            DuplicateAndRedirect(
                source_serial=200, per_pred_targets=((10, 50),),
            ),
        ]
        frag = self._finalize(mods, owned_blocks={100, 200})
        kinds = sorted(type(m).__name__ for m in frag.modifications)
        assert kinds == ["DuplicateAndRedirect", "RedirectGoto"]

    def test_dup_and_redirect_contributes_linearization_decision(self) -> None:
        # uee-dnhk cumulative-view extension: DupAndRedirect's source must
        # appear as a LinearizationDecision in the contribution so cross-
        # fragment Mode 1 detection picks up later contradictory
        # RedirectGotos.
        mods = [
            DuplicateAndRedirect(
                source_serial=100, per_pred_targets=((99, 2), (98, 21)),
        ...
        frag = self._finalize(mods, owned_blocks={100})
        ctx = frag.metadata[PLANNER_CTX_METADATA_KEY]
        # First per_pred target is the linearization target (the source
        # block's goto is mutated to that target by DupAndRedirect).
        assert len(ctx.linearizations) == 1
        decision = ctx.linearizations[0]
        assert decision.src == 100
        assert decision.tgt == 2  # per_pred_targets[0][1]
        assert "duplicate_and_redirect" in decision.reason
        # Source is also claimed.
        assert 100 in ctx.claimed_sources

    def test_dup_makes_view_detect_later_conflicting_redirect(self) -> None:
        # End-to-end: SRW emits DupAndRedirect for blk[100], a later
        # strategy (same pipeline) emits RedirectGoto src=100 -> different
        # target.  The cumulative_planner_view picks up SRW's DupAndRedirect
        # as a linearization, and the Mode 1 filter drops the later
        # RedirectGoto.
        srw_frag = self._finalize(
            [
                DuplicateAndRedirect(
                    source_serial=100,
                    per_pred_targets=((99, 2), (98, 21)),
                ),
            ],
            owned_blocks={100},
        )
        view = CumulativePlannerView.compile([srw_frag])
        # Verify the view sees DupAndRedirect's first target as the
        # linearization for blk[100].
        assert view.is_linearized(100) is True
        assert view.linearization_target_for(100) == 2
        assert view.is_claimed(100) is True

        # Now a later strategy queues a RedirectGoto src=100 -> 99 (a
        # different target).  finalize_reconstruction_fragment with the
        # cumulative view drops it (Mode 1 first-fragment-wins).
        later_frag = finalize_reconstruction_fragment(
            strategy_name="later_strategy",
            modifications=[
                RedirectGoto(from_serial=100, old_target=2, new_target=99),
                # Unrelated mod on a different source — kept.
                RedirectGoto(from_serial=200, old_target=2, new_target=201),
            ],
            owned_blocks={100, 200},
            owned_edges=frozenset(),
            accepted_metadata=[],
            rejected_metadata=[],
            allow_post_apply_bst_cleanup=True,
        ...
        srcs = {m.from_serial for m in later_frag.modifications if isinstance(m, RedirectGoto)}
        assert srcs == {200}, (
            "blk[100] RedirectGoto must be dropped because a prior fragment "
            "already committed via DupAndRedirect"
        )

    def test_no_dup_means_redirect_passes_through(self) -> None:
        # Baseline: a RedirectGoto with no DupAndRedirect on the same
        # source is kept untouched.
        mods = [RedirectGoto(from_serial=50, old_target=2, new_target=51)]
        frag = self._finalize(mods, owned_blocks={50})
        assert len(frag.modifications) == 1
        assert isinstance(frag.modifications[0], RedirectGoto)

    def test_multiple_redirects_all_dropped_when_dup_present(self) -> None:
        # A handful of RedirectGotos targeting the same shared block all
        # get dropped — DupAndRedirect alone survives.  Real-world tracer
        # data showed blk[32] with 2 RedirectGotos + 1 DupAndRedirect.
        mods = [
            RedirectGoto(from_serial=32, old_target=2, new_target=62),
            RedirectGoto(from_serial=32, old_target=3, new_target=62),
            DuplicateAndRedirect(
                source_serial=32, per_pred_targets=((24, 62), (31, 62)),
            ),
            # Unrelated mod on a different source — kept.
            RedirectGoto(from_serial=999, old_target=2, new_target=998),
        ]
        frag = self._finalize(mods, owned_blocks={32, 999})
        # Two mods survive: the DupAndRedirect, and the unrelated RedirectGoto.
        kinds = [type(m).__name__ for m in frag.modifications]
        assert kinds.count("DuplicateAndRedirect") == 1
        assert kinds.count("RedirectGoto") == 1
        srcs_redirect = {
