...
def test_preopt_setcc_table_accepts_scaled_lookup_suffix(monkeypatch) -> None:
    monkeypatch.setattr(detached_handler_island.ida_hexrays, "mop_t", _Operand)
    plan = build_rhad_generated_reference_plan(
        native_key=make_native_key(
            input_identity=f"sha256:{RHAD_INPUT_SHA256}",
            function_rva=0xA560,
    ...
    overflow_result = _Operand(ida_hexrays.mop_r, register=60, size=1)
    sign_result = _Operand(ida_hexrays.mop_r, register=61, size=1)
    xor_result = _Operand(ida_hexrays.mop_r, register=62, size=1)
    predicate_result = _Operand(ida_hexrays.mop_r, register=7, size=4)
    xor = _Instruction(
        ida_hexrays.m_xor,
        0x40A786,
    ...
    scaling = _Instruction(
        ida_hexrays.m_mul,
        0x40A789,
        left=_Operand(ida_hexrays.mop_n, value=8, size=4),
        right=predicate_result,
    )
    lookup = _Instruction(
        ida_hexrays.m_ldx,
        0x40A789,
        right=_Operand(
            ...
            nested=_Instruction(
                ida_hexrays.m_add,
                0x40A789,
                left=_Operand(ida_hexrays.mop_v, target_ea=0x48B4F8, size=4),
                right=_Operand(
                    ida_hexrays.mop_d,
                    size=4,
                    nested=scaling,
...
def test_preopt_setcc_table_accepts_exact_equality_suffix() -> None:
    plan = build_rhad_generated_reference_plan(
        native_key=make_native_key(
            input_identity=f"sha256:{RHAD_INPUT_SHA256}",
            function_rva=0xA560,
    ...
    instructions = (
        _Instruction(
            ...
            right=_Operand(ida_hexrays.mop_n, value=0x13B0D3B2, size=4),
        ...
        _Instruction(
            ida_hexrays.m_mul,
            0x40AE31,
            left=_Operand(ida_hexrays.mop_n, value=0x40, size=4),
            right=index_result,
            dest=index_result,
        ),
        _Instruction(ida_hexrays.m_ldx, 0x40AE34),
        _Instruction(ida_hexrays.m_cfadd, 0x40AE3A),
        _Instruction(ida_hexrays.m_ofadd, 0x40AE3A),
        _Instruction(ida_hexrays.m_setz, 0x40AE3A),
        _Instruction(ida_hexrays.m_setp, 0x40AE3A),
        _Instruction(ida_hexrays.m_sets, 0x40AE3A),
        _Instruction(ida_hexrays.m_add, 0x40AE3A),
        _Instruction(
            ida_hexrays.m_icall,
            0x40AE3C,
    ...
    template_block = detached_handler_island.DetachedSnippetBlockTemplate(
        source_serial=0,
        native_entry_ea=0x40AE26,
        native_end_ea=0x40AE3E,
        instructions=instructions,
        block_type=ida_hexrays.BLT_1WAY,
        block_flags=ida_hexrays.MBL_TCAL,
        successor_serials=(1,),
