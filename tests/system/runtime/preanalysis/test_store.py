...
class TestReconStoreSaveLoad:
    ...
    def test_primary_key_upsert(self, store, sample_result):
        ...
        store.save_preanalysis_result(updated)
        loaded = store.load_preanalysis_results(func_ea=0x401000, maturity=5)
        assert len(loaded) == 1
        assert loaded[0].metrics["block_count"] == 30

    def test_load_returns_empty_for_unknown(self, store):
        results = store.load_preanalysis_results(func_ea=0xDEAD, maturity=5)
        assert results == []
    ...
    def test_load_latest_preanalysis_result_prefers_highest_maturity(self, store):
        ...
        store.save_preanalysis_result(older)
        store.save_preanalysis_result(newer)

        loaded = store.load_latest_preanalysis_result(
            func_ea=0x401000,
            collector_name="handler_transitions",
        )
    ...
    def test_load_latest_preanalysis_result_can_filter_exact_maturity(self, store):
        ...
        store.save_preanalysis_result(first)
        store.save_preanalysis_result(second)
...
class TestReconStoreHints:
    ...
    def test_clear_func(self, store, sample_result, sample_hints):
        ...
        assert store.load_preanalysis_results(func_ea=0x401000, maturity=5) == []
