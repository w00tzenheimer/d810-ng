...
def mock_ida_modules():
    ...
    # Mock mop_t as a real class so functools.singledispatch can register it
    class MockMopT:
        def __init__(self, arg=None):
            if arg is not None:
                self.t = getattr(arg, 't', 0)
                self.nnn = getattr(arg, 'nnn', MagicMock(value=0))
                self.size = getattr(arg, 'size', 4)
            else:
                self.t = 0
                self.nnn = MagicMock(value=0)
                self.size = 4

    # Mock minsn_t as a real class for singledispatch registration
    class MockMinsnT:
        pass
