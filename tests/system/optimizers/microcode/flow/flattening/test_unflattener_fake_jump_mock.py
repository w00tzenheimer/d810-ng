...
def _patch_mop_t_constructor():
    """Patch ida_hexrays.mop_t constructor to prevent hangs.

    The real mop_t constructor hangs when called with a MagicMock argument.
    Since analyze_blk calls ida_hexrays.mop_t(blk.tail.l) and our blocks
    are mocked, we need to intercept this call.
    """
    with patch('ida_hexrays.mop_t', side_effect=lambda *args, **kwargs: MagicMock()):
        yield
...
def create_mock_block(serial=0, tail_opcode=None, reginsn_qty=1, predset=None):
    ...
    if tail_opcode is not None:
        ...
        block.tail.r.t = ida_hexrays.mop_n  # numeric operand type
...
class TestAnalyzeBlk:
    ...
    def test_analyze_blk_no_tail_returns_zero(self, unflatten_rule):
    ...
    def test_analyze_blk_wrong_opcode_returns_zero(self, unflatten_rule):
        """Block with non-fake-jump opcode returns 0 changes."""
        # m_nop is not in FAKE_LOOP_OPCODES
        block = create_mock_block(serial=1, tail_opcode=ida_hexrays.m_nop)

        result = unflatten_rule.analyze_blk(block)

        assert result == 0

    def test_analyze_blk_multiple_regular_instructions_returns_zero(self, unflatten_rule):
        """Block with multiple regular instructions returns 0 changes."""
        block = create_mock_block(
            serial=1,
            tail_opcode=ida_hexrays.m_jz,
            reginsn_qty=3
        )
    ...
    def test_analyze_blk_non_numeric_operand_returns_zero(self, unflatten_rule):
        """Block with non-numeric right operand returns 0 changes."""
        block = create_mock_block(serial=1, tail_opcode=ida_hexrays.m_jz)
        block.tail.r.t = ida_hexrays.mop_r  # register, not numeric

        result = unflatten_rule.analyze_blk(block)

        assert result == 0

    def test_analyze_blk_no_predecessors(self, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_analyze_blk_unresolved_path_returns_zero(self, mock_tracker_class, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.get_all_possibles_values')
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_analyze_blk_none_in_values_returns_zero(self, mock_tracker_class, mock_get_values, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.get_all_possibles_values')
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_analyze_blk_successful_single_predecessor(self, mock_tracker_class, mock_get_values, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.get_all_possibles_values')
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_analyze_blk_multiple_predecessors(self, mock_tracker_class, mock_get_values, unflatten_rule):
...
class TestFixSuccessor:
    ...
    def test_fix_successor_empty_values_returns_false(self, unflatten_rule):
        """Empty comparison values list returns False."""
        fake_loop_block = create_mock_block(serial=1, tail_opcode=ida_hexrays.m_jz)
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.change_1way_block_successor')
    def test_fix_successor_jz_always_taken(self, mock_change_succ, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.change_1way_block_successor')
    def test_fix_successor_jz_never_taken(self, mock_change_succ, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.change_1way_block_successor')
    def test_fix_successor_jnz_always_taken(self, mock_change_succ, unflatten_rule):
        """m_jnz with all values not equal to compared value -> jump taken."""
        fake_loop_block = create_mock_block(serial=1, tail_opcode=ida_hexrays.m_jnz)
        fake_loop_block.tail.r.nnn.value = 42
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.change_1way_block_successor')
    def test_fix_successor_jnz_never_taken(self, mock_change_succ, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.change_1way_block_successor')
    def test_fix_successor_mixed_values_returns_false(self, mock_change_succ, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.dump_microcode_for_debug')
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.change_1way_block_successor')
    def test_fix_successor_with_dump_enabled(self, mock_change_succ, mock_dump, unflatten_rule):
...
class TestOptimize:
    ...
    def test_optimize_rule_should_not_be_used(self, unflatten_rule):
    ...
    def test_optimize_no_changes_made(self, unflatten_rule):
        """Optimize returns 0 when analyze_blk makes no changes."""
        block = create_mock_block(serial=1)

        with patch.object(unflatten_rule, 'check_if_rule_should_be_used', return_value=True), \
             patch.object(unflatten_rule, 'analyze_blk', return_value=0):
            result = unflatten_rule.optimize(block)

        assert result == 0
        block.mba.mark_chains_dirty.assert_not_called()

    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.safe_verify')
    def test_optimize_successful_changes(self, mock_verify, unflatten_rule):
        """Optimize applies changes and marks chains dirty."""
        block = create_mock_block(serial=1)

        with patch.object(unflatten_rule, 'check_if_rule_should_be_used', return_value=True), \
             patch.object(unflatten_rule, 'analyze_blk', return_value=2):
            result = unflatten_rule.optimize(block)
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.safe_verify')
    def test_optimize_verify_called_with_correct_args(self, mock_verify, unflatten_rule):
        """Verify that safe_verify is called with correct arguments."""
        block = create_mock_block(serial=1)

        with patch.object(unflatten_rule, 'check_if_rule_should_be_used', return_value=True), \
             patch.object(unflatten_rule, 'analyze_blk', return_value=1):
...
class TestEdgeCases:
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.get_all_possibles_values')
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_tracker_initialization(self, mock_tracker_class, mock_get_values, unflatten_rule):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.get_all_possibles_values')
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_get_all_possibles_values_called_correctly(self, mock_tracker_class, mock_get_values, unflatten_rule):
    ...
    def test_multiple_paths_with_different_resolution_majority_resolved(
        self, mock_tracker_class, mock_get_values, unflatten_rule
    ):
    ...
    def test_multiple_paths_with_different_resolution_majority_unresolved(
        self, mock_tracker_class, unflatten_rule
    ):
    ...
    @patch('d810.optimizers.microcode.flow.flattening.unflattener_fake_jump.MopTracker')
    def test_safety_check_prevents_unsafe_redirect(self, mock_tracker_class, unflatten_rule):
...
class TestConstants:
    ...
    def test_fake_loop_opcodes(self):
