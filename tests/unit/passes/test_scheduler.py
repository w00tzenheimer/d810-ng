...
def test_scheduler_drains_at_or_after_requested_maturity_and_removes_entries():
    ...
    assert (
        scheduler.drain(func_ea=0x1000, current_maturity=IRMaturity.GLOBAL_ANALYZED)
        == ()
    )
    drained = scheduler.drain(
    ...
    assert (
        scheduler.drain(func_ea=0x1000, current_maturity=IRMaturity.VARIABLE_RECOVERED)
        == ()
    )
...
def test_scheduler_reset_all_clears_everything():
    ...
    assert (
        scheduler.drain(func_ea=0x2000, current_maturity=IRMaturity.GLOBAL_ANALYZED)
        == ()
    )
