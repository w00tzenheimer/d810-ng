...
def test_all_bundled_constant_profiles_are_canonical_and_compile() -> None:
    ...
    for path, document, entry in entries:
        ...
        assert {
            stage.stage_id for stage in schedule.stages if not stage.enabled
        } == EXPECTED_DISABLED_STAGES.get(path.name, frozenset())
...
def test_flatfold_migration_preserves_disabled_forward_constants_stage() -> None:
    ...
    assert schedule.stage("fold-readonly-data").enabled is True
    assert schedule.stage("fold-constant-subtree").enabled is True
    assert schedule.stage("forward-constants").enabled is False
