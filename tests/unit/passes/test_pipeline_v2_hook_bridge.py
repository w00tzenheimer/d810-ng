...
def test_constant_simplification_bundle_expands_to_private_live_stages():
    ...
    activation = compile_config_v2_hook_schedule(project)

    assert activation.configured_pass_ids == ("constant-simplification",)
    assert [rule.name for rule in activation.instruction_bindings] == [
        "FoldReadonlyDataRule",
        "ConstantSubtreeFoldRule",
    ]
    assert activation.instruction_bindings[0].config == {
        "maturities": [
            "MMAT_PREOPTIMIZED",
            "MMAT_LOCOPT",
            "MMAT_CALLS",
    ...
    assert [rule.name for rule in activation.block_bindings] == [
        "ForwardConstantPropagationRule",
    ]
    assert activation.block_bindings[0].config == {
        "maturities": [
            "MMAT_CALLS",
            "MMAT_GLBOPT1",
            "MMAT_GLBOPT2",
...
def test_default_instruction_only_bridge_derives_rules_from_pipeline_v2_only():
    ...
    assert len(activation.instruction_bindings) == 180
    assert activation.instruction_bindings[0].name == "FoldReadonlyDataRule"
    assert activation.instruction_bindings[1].name == "ConstantSubtreeFoldRule"
    assert all(
        rule.name != "CopiedLegacyInstructionRule"
        for rule in activation.instruction_bindings
    )
    assert [rule.name for rule in activation.block_bindings] == [
        "ForwardConstantPropagationRule",
        "JumpFixer",
    ]
    assert all(rule.name != "CopiedLegacyBlockRule" for rule in activation.block_bindings)
    jump_fixer = activation.block_bindings[-1]
...
def test_hodur_bridge_derives_unflattener_trigger_and_simple_flow_rule():
    ...
    assert activation.instruction_bindings == ()
    assert [rule.name for rule in activation.block_bindings] == [
        STATE_MACHINE_UNFLATTENER_RULE,
        "JumpFixer",
    ]
    unflattener = activation.block_bindings[0]
...
def test_state_machine_plus_constant_bundle_schedules_one_late_flow_fold():
    ...
    assert activation.block_bindings[1].config == {
...
def test_state_cff_bridge_accepts_typed_direct_threshold_on_the_complete_spine():
    ...
    assert activation.block_bindings[0].config == {"min_state_constant": 0x8000}
...
def test_identity_call_bridge_derives_explicit_opt_in_rule_config():
    ...
    assert activation.instruction_bindings == ()
    assert [rule.name for rule in activation.block_bindings] == ["IdentityCallResolver"]
    assert activation.block_bindings[0].config == {
...
def test_indirect_branch_call_bridge_derives_explicit_flow_rules():
    ...
    assert [rule.config for rule in activation.block_bindings] == [{}, {}]
    assert all(rule.name != "CopiedLegacyBlockRule" for rule in activation.block_bindings)
