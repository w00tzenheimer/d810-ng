...
def _diag_tree_mov(
    *,
    ...
    """A meta-rich (operand-tree) ``mov`` diag row.

    Routed through ``project_diag_instruction`` -- ``dest_stkoff`` and
    ``source_stkoffs`` are derived from the canonical projection of the ``meta``
    operand tree, NOT the flat fields (left empty here on purpose).
    """
    import json
    ...
    return InstructionSnapshot(
        ...
        src_r_value=None,
        dstr=dstr,
        meta=json.dumps({"l": _meta_stack(src_stkoff), "d": _meta_stack(dest_stkoff)}),
    )
...
def _diag_predicate(
    *,
    ...
    """A meta-less conditional-branch diag row.

    A diag predicate's ``opcode_name`` (``m_jnz``) is NOT in
    ``_OPCODE_NAME_TO_INSN_KIND``, so an operand-tree projection cannot recover
    the ``CONDITIONAL_BRANCH`` transfer.  The production meta-less predicate row
    carries the transfer as a ``meta`` attr instead; the byte-identical legacy
    flat path reads it and the flat predicate-input stkoffs.
    """
    import json
    ...
    return InstructionSnapshot(
        ...
        src_r_value=None,
        dstr=dstr,
        meta=json.dumps({"control_transfer": "conditional_branch"}),
    )


def test_detects_carrier_from_operand_tree_diag_rows() -> None:
    """Carrier detection driven by ``project_diag_instruction`` (S7).

    The carrier-input writers (blocks 52/88) and the carrier writers (blocks
    151/186) carry ``meta`` operand trees, so their ``dest_stkoff`` /
    ``source_stkoffs`` are derived from the canonical diag-row projection.  The
    conditional-branch predicate (block 81) is the meta-less legacy row.  The
    SAME ``LOOP_CARRIER_WRITER_OUTSIDE_SCC`` fact the FlowGraph shape produces is
    recovered from the diag-operand-tree currency.
    """
    facts = _collect(
        _target(
            _block(
            ...
            _block(
                ...
                _diag_predicate(
                    index=0,
                    ea=0x180014081,
                    dstr="jnz (%var_528.8 + %var_508.8), @83",
                    src_l_stkoff=0x528,
                    src_r_stkoff=0x508,
                ),
                succs=(82, 83),
...
def test_meta_less_attrs_only_rows_yield_no_fact() -> None:
    ...
    facts = _collect(
        _target(
            _block(
                81,
                _attrs_only(0, 0x180014081, "m_jnz"),
                succs=(82, 83),
            ),
            _block(82, succs=(81,)),
            _block(83, succs=()),
