...
from d810.analyses.control_flow.condition_chain_model import is_terminal_handler
...
def test_terminal_handler_accepts_all_no_successor_paths() -> None:
    ...
    assert is_terminal_handler(fg, 1, dispatcher_serial=9, condition_chain_blocks=set())
...
def test_terminal_handler_rejects_path_to_dispatcher() -> None:
    ...
    assert not is_terminal_handler(
        fg, 1, dispatcher_serial=9, condition_chain_blocks=set()
    )
...
def test_terminal_handler_rejects_path_to_bst_block() -> None:
    ...
    assert not is_terminal_handler(
        fg, 1, dispatcher_serial=9, condition_chain_blocks={4}
    )
