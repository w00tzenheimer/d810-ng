...
class TestDebugScope:
    ...
    def test_debug_scope_restores_level(self, temp_db):
        ...
        with debug_scope(loggers="test_restore", db_path=temp_db, level=logging.DEBUG):
    ...
    def test_debug_scope_yields_handler(self, temp_db):
        ...
        with debug_scope(
            loggers="test_yield", db_path=temp_db, test_id="yield_test"
        ) as handler:
...
class TestQueryLogs:
    ...
    def test_query_logs_filters(self, populated_db):
        ...
        logger1_debug = query_logs(
            populated_db, logger="module.submodule1", level="DEBUG"
        )
