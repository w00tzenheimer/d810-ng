...
def test_cfg_transaction_authority_and_creation_witness_survive_restart(
    ...
    for phase_index, phase in enumerate(phases):
        emit(
            CfgTransactionAttemptObserved(
                ...
                mba_generation=8,
                evidence_generation=3,
                mutation_started=phase_index >= 4,
                poisoned=False,
                creation_witnesses=(
                    CfgCreationWitnessObserved(
                        ...
                        insertion_quantity_before=(12 if phase_index >= 4 else None),
                        insertion_quantity_after=(13 if phase_index >= 4 else None),
                        requested_insertion_serial=(7 if phase_index >= 4 else None),
                        returned_serial=(8 if phase_index >= 4 else None),
                        state=("committed" if phase == "committed" else "reserved"),
                    ),
                    CfgCreationWitnessObserved(
                        local_block_id="observed",
                        provenance="observed_ephemeral",
                        reserved_handle_token="handle-observed",
                        logical_proxy_token="proxy-observed",
                        logical_version=2,
                        logical_generation=8,
                        state="observed",
                    ),
    ...
    assert diag_conn.execute(
        "SELECT current_phase,mutation_started,poisoned,first_failure_reason "
        "FROM cfg_transaction_attempts WHERE plan_id=? AND attempt_id=?",
        ("portable-plan", "attempt-1"),
    ).fetchone() == ("committed", 1, 0, None)
    assert diag_conn.execute(
...
def test_cfg_transaction_persists_only_the_first_failure(diag_conn) -> None:
    emit(
        CfgTransactionAttemptObserved(
            ...
            phase="rejected_clean",
            phase_index=0,
            mba_generation=2,
            evidence_generation=1,
            mutation_started=False,
            poisoned=False,
            first_failure_obligation="graph-closure",
            first_failure_phase="preflight",
            first_failure_reason="first",
        )
    )
    emit(
        CfgTransactionAttemptObserved(
            ...
            poisoned=True,
            first_failure_obligation="later",
            first_failure_phase="realizing",
            first_failure_reason="later failure",
        )
    )

    assert diag_conn.execute(
        "SELECT first_failure_obligation,first_failure_phase,"
        "first_failure_reason,mutation_started,poisoned "
        "FROM cfg_transaction_attempts WHERE plan_id='failed-plan'"
    ).fetchone() == ("graph-closure", "preflight", "first", 1, 1)


def test_committed_cfg_creation_mapping_is_queryable_after_database_restart(
    tmp_path,
) -> None:
    path = tmp_path / "cfg-authority.sqlite3"
