...
def test_patch_plan_has_no_transitional_or_raw_block_references() -> None:
    ...
    for operation_type in typing.get_args(patch_plan.PatchOperation):
        ...
        for field_name, annotation in annotations.items():
            if field_name != "pred_arm" and any(token in field_name for token in block_field_tokens) and contains_raw_int(
                annotation
            ):
...
def test_every_production_patch_plan_compiler_call_names_exact_block_authority() -> None:
    ...
    for relative, call in _production_calls():
        ...
        if relative == "transforms/plan.py":
            continue
        if not any(
            keyword.arg == "block_refs_by_serial" for keyword in call.keywords
        ):
            violations.append(f"{relative}:{call.lineno}")
    assert violations == []
...
def test_semantic_sdk_creation_sites_are_exhaustive_and_receipt_backed() -> None:
    ...
    for method_name, method in methods.items():
        if "semantic" not in method_name and method_name != "_build_fallthrough_goto_helper":
            continue
...
def test_synthetic_identity_is_nominal_not_derived_from_live_coordinates() -> None:
    ...
    constructors = {
        ...
        if isinstance(node, ast.FunctionDef)
        and node.name in {"synthetic", "created_synthetic", "observed_ephemeral"}
    }
