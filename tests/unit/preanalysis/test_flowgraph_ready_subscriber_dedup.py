...
class _CountingCollector:
    ...
    def collect(self, target, func_ea: int, maturity: int) -> PreanalysisResult:
...
class TestFlowGraphReadyCoordinatorDedup:
    ...
    def _build_phase(
        ...
        monkeypatch.setattr(
            d810.passes.phase, "get_preanalysis_writer", lambda _: _NoopWriter()
        )

        phase = PreanalysisPhase(store=_StubStore())  # type: ignore[arg-type]
    ...
    def _subscriber(
        self,
        phase: PreanalysisPhase | None = None,
        *,
...
class TestNoDirectCollectionInAdapterGates:
    ...
    def test_no_direct_pre_d810_fact_capture_call_in_hexrays_hooks(
        ...
        src = hexrays_hooks_path.read_text()
        assert "self._analysis_runtime.capture_maturity_facts(" not in src, (
            "hexrays_hooks.py must not call "
            "self._analysis_runtime.capture_maturity_facts(...) directly. "
