...
from d810.passes.fact_runtime import FactCollectionResult, PreanalysisFactRuntime
...
def test_capture_persists_collector_observations_when_snapshot_is_available() -> None:
    ...
    def _persist(snapshot, func_ea, observations, mappings, conflicts) -> None:
        calls.append((snapshot, func_ea, observations, mappings, conflicts))

    runtime = PreanalysisFactRuntime(persistence_callback=_persist)
...
def test_find_block_for_ea_supports_flow_graph_snapshots() -> None:
    ...
    assert PreanalysisFactRuntime._find_block_for_ea(flow_graph, 0x401234) == 42
    assert PreanalysisFactRuntime._find_block_for_ea(flow_graph, 0xDEADBEEF) is None
...
def test_capture_summary_log_uses_maturity_name(monkeypatch) -> None:
    ...
    capture_lines = [
        message for message in messages if message.startswith("FACT_LIFECYCLE_CAPTURE")
    ]
...
def test_validated_view_accumulates_observations_and_filters_stale_mappings() -> None:
    ...
    runtime.capture(
        object(), func_ea=0x401000, provider_phase=_phase(1), phase="pre_d810"
    )
    active_before = runtime.validated_view(0x401000, 1)
    runtime.capture(
        object(), func_ea=0x401000, provider_phase=_phase(2), phase="pre_d810"
    )
...
def test_validated_view_is_historically_scoped() -> None:
    ...
    runtime.capture(
        object(), func_ea=0x401000, provider_phase=_phase(3), phase="pre_d810"
    )
...
def test_induction_fact_absence_creates_identity_lost_mapping() -> None:
    ...
    assert mapping.target_maturity == PreanalysisFactRuntime._provider_level_text(2)
...
def test_induction_fact_remaps_on_stable_source_ea_mop_continuity() -> None:
    ...
    assert mapping.target_mop_signature == ("mop_S:writeback:dest=0x638:source=0x468:8")
...
def test_induction_identity_lost_mapping_is_not_duplicated() -> None:
    ...
    runtime.capture(
        object(), func_ea=0x401000, provider_phase=_phase(2), phase="post_d810"
    )
        ...
        def collect(self, target, *, func_ea: int, maturity: int, phase: str):
            ...
            return (
                FactObservation(
                    ...
                    mop_signature=(
                        "terminal_byte_emit:byte=2:dest=%var_dst.8:counter=%var_53.8"
                    ),
...
def test_generic_structural_fact_gets_identity_lost_mapping_when_collector_ran() -> (
    None
):
    class _InitialCollector:
...
def test_generic_structural_fact_does_not_emit_loss_when_collector_did_not_run() -> (
    None
):
    class _InitialCollector:
    ...
    assert view.mappings == ()
    assert {obs.fact_id for obs in view.active_observations} == {"return_frontier:old"}
...
def test_state_write_anchor_rewrite_emits_state_const_rewritten_mapping() -> None:
    ...
    runtime.capture(
        object(),
        func_ea=0x401000,
        provider_phase=_phase(locopt, "MMAT_LOCOPT"),
        phase="pre_d810",
    )
    runtime.capture(
        object(),
        func_ea=0x401000,
        provider_phase=_phase(glbopt1, "MMAT_GLBOPT1"),
        phase="pre_d810",
    )
    ...
    assert any(obs.maturity == "MMAT_LOCOPT" for obs in view.active_observations)
...
def test_state_write_anchor_same_const_does_not_emit_rewrite() -> None:
    ...
    assert not any(m.status is FactStatus.STATE_CONST_REWRITTEN for m in view.mappings)
...
def test_state_write_anchor_ea_changed_canonical_stkoff_emits_rewrite() -> None:
    ...
    runtime.capture(
        object(),
        func_ea=0x401000,
        provider_phase=_phase(calls, "MMAT_CALLS"),
        phase="pre_d810",
    )
