...
def _dispatch_map(
    states: tuple[int, ...] = TIGRESS_VISIBLE_STATES,
) -> StateDispatcherMap:
...
def test_tigress_minmaxarray_acceptance_transitions() -> None:
    ...
    assert {row.state_const for row in _dispatch_map().rows} == set(
        TIGRESS_VISIBLE_STATES
    )
    transitions = {
        fact.source_state: fact.next_states
        for fact in facts
        if fact.transition_kind
        in {
...
def test_conditional_cases_require_valid_states_and_source_predicate() -> None:
    ...
    unresolved = collect_switch_case_transition_facts(
        ...
        case_bodies=(
            SwitchCaseBody(
                state=4, entry_block=104, state_writes=(9, 13), source_predicate=True
            ),
        ),
    )[0]
...
def test_direct_case_facts_carry_exit_block_and_ordered_path() -> None:
    ...
    direct = next(
        fact
        for fact in facts
        if fact.transition_kind == SwitchCaseTransitionKind.DIRECT
    )
