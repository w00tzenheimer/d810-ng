...
def test_exports_typed_plan_references_for_native_and_observed_blocks() -> None:
    native = StableBlockIdentity.from_intervals(
        (NativeEaInterval(0x40D348, 0x40D349),), native_key=NATIVE_KEY
    )
    graph = FlowGraph(
        blocks={
            0: BlockSnapshot(0, 0, (), (), 0, 0x40D348, ()),
            1: BlockSnapshot(1, 0, (), (), 0, 0xFFFFFFFFFFFFFFFF, ()),
        },
        entry_serial=0,
        func_ea=0x40D348,
    )
    index = MbaBlockIdentityIndex.from_flow_graph(
        ...
        native_key=NATIVE_KEY,
        flow_graph=graph,
        session_id="typed-plan-refs",
    )

    native_ref = index.plan_ref_for_serial(0)
    observed_ref = index.plan_ref_for_serial(1)

    assert isinstance(native_ref, NativeBlockRef)
    assert native_ref.identity.native_ranges == native.native_ranges
    assert isinstance(observed_ref, LogicalBlockRef)
    assert observed_ref.session_id == "typed-plan-refs"
    assert observed_ref.proxy_token
    assert observed_ref.version == 0
...
def test_reserved_synthetic_proxy_binds_only_when_insertion_is_realized() -> None:
    ...
    attempt_id = TransactionAttemptId("plan-a", transaction_id)
    plan_ref = PlanBlockRef("plan-a", "helper")
    index.begin_transaction(transaction_id, 2)
    reservation = index.reserve_plan_block(attempt_id, plan_ref)
...
def test_plan_block_replay_allocates_new_attempt_local_realization() -> None:
    plan_ref = PlanBlockRef("stable-plan", "same-body-a")
    index = MbaBlockIdentityIndex.from_bindings(
    ...
    first_attempt = TransactionAttemptId("stable-plan", "attempt-1")
    index.begin_transaction(first_attempt.attempt_id, 0)
    first = index.reserve_plan_block(first_attempt, plan_ref)
    first_receipt = index.bind_reserved_plan_block(
        first_attempt, plan_ref, insertion_serial=0, returned_serial=0
    )
    index.commit_proxy_transaction(first_attempt.attempt_id)
    index.advance_generation()

    second_attempt = TransactionAttemptId("stable-plan", "attempt-2")
    index.begin_transaction(second_attempt.attempt_id, 1)
    ...
    assert first_receipt.plan_ref == plan_ref
...
def test_plan_block_receipt_preserves_requested_and_returned_coordinates() -> None:
    ...
    original = index.handle_for_serial(4)
    assert original is not None
    attempt = TransactionAttemptId("plan-a", "attempt-divergent-coordinate")
    ref = PlanBlockRef("plan-a", "created")
    index.begin_transaction(attempt.attempt_id, 5)
    index.reserve_plan_block(attempt, ref)
...
def test_plan_block_reservation_rejects_wrong_authority_and_duplicates() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        session_id="session-a", generation=1, bindings=(), native_key=NATIVE_KEY
    )
    attempt = TransactionAttemptId("plan-a", "attempt-a")
    ref = PlanBlockRef("plan-a", "new-block")
    index.begin_transaction(attempt.attempt_id, 0)
    index.reserve_plan_block(attempt, ref)

    with pytest.raises(ValueError, match="already reserved"):
        index.reserve_plan_block(attempt, ref)
    with pytest.raises(ValueError, match="plan authority"):
        index.reserve_plan_block(attempt, PlanBlockRef("plan-b", "new-block"))
    with pytest.raises(ValueError, match="active identity transaction"):
        index.reserve_plan_block(
            TransactionAttemptId("plan-a", "other-attempt"), ref
        )


def test_identical_plan_bodies_still_receive_distinct_nominal_owners() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        session_id="session-a", generation=1, bindings=(), native_key=NATIVE_KEY
    )
    attempt = TransactionAttemptId("plan-a", "attempt-a")
    index.begin_transaction(attempt.attempt_id, 0)
    first = index.reserve_plan_block(attempt, PlanBlockRef("plan-a", "body-1"))
    second = index.reserve_plan_block(attempt, PlanBlockRef("plan-a", "body-2"))

    assert first.logical_version.handle != second.logical_version.handle
    assert first.logical_version.version_id != second.logical_version.version_id


def test_plan_reservation_cannot_cross_session_or_generation() -> None:
    attempt = TransactionAttemptId("plan-a", "attempt-a")
    ref = PlanBlockRef("plan-a", "new-block")
    source = MbaBlockIdentityIndex.from_bindings(
        session_id="source", generation=1, bindings=(), native_key=NATIVE_KEY
    )
    source.begin_transaction(attempt.attempt_id, 0)
    source.reserve_plan_block(attempt, ref)

    foreign = MbaBlockIdentityIndex.from_bindings(
        session_id="foreign", generation=1, bindings=(), native_key=NATIVE_KEY
    )
    foreign.begin_transaction(attempt.attempt_id, 0)
    with pytest.raises(ValueError, match="reserved plan owner"):
        foreign.bind_reserved_plan_block(
            attempt, ref, insertion_serial=0, returned_serial=0
        )

    source.generation = 2
    with pytest.raises(ValueError, match="stale MBA generation"):
        source.bind_reserved_plan_block(
            attempt, ref, insertion_serial=0, returned_serial=0
        )


def test_aborted_plan_block_reservation_never_resolves() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        session_id="session-a", generation=2, bindings=(), native_key=NATIVE_KEY
    )
    attempt = TransactionAttemptId("plan-a", "attempt-abort")
    ref = PlanBlockRef("plan-a", "aborted")
    index.begin_transaction(attempt.attempt_id, 0)
    reservation = index.reserve_plan_block(attempt, ref)
    index.abort_proxy_transaction(attempt.attempt_id)

    assert index.resolve(reservation.logical_version.handle) is None
    with pytest.raises(ValueError, match="active identity transaction"):
        index.bind_reserved_plan_block(
            attempt, ref, insertion_serial=0, returned_serial=0
        )
...
def test_serial_space_imports_are_observed_not_created() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        generation=3, bindings=(), native_key=NATIVE_KEY
    )
...
def test_record_insert_rejects_unreserved_created_synthetic_owner() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        generation=3, bindings=(), native_key=NATIVE_KEY
    )
    index.begin_transaction("unreserved", 0)
    created = MbaBlockHandle.created_synthetic(
        session_id=index.session_id,
        token="unreserved-created",
    )
    index._handles_by_token[created.token] = created

    with pytest.raises(ValueError, match="bind_reserved_plan_block"):
        index.record_insert(
            transaction_id="unreserved",
            insertion_serial=0,
            created=created,
            returned_serial=0,
        )
...
def test_generic_insert_cannot_consume_a_typed_plan_reservation() -> None:
    ...
    original = index.handle_for_serial(2)
    assert original is not None
    attempt = TransactionAttemptId("plan-a", "typed-only")
    ref = PlanBlockRef("plan-a", "created")
...
def test_transaction_rejects_same_raw_attempt_under_another_plan() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        session_id="session-a", generation=1, bindings=(), native_key=NATIVE_KEY
    )
    index.begin_transaction("shared-attempt", 0)
    index.reserve_plan_block(
        TransactionAttemptId("plan-a", "shared-attempt"),
        PlanBlockRef("plan-a", "first"),
    )

    with pytest.raises(ValueError, match="transaction attempt authority"):
        index.reserve_plan_block(
            TransactionAttemptId("plan-b", "shared-attempt"),
            PlanBlockRef("plan-b", "second"),
        )


def test_divergent_plan_insert_rollback_restores_requested_coordinate_shift() -> None:
    first = StableBlockIdentity.from_intervals(
        (NativeEaInterval(0x401000, 0x401010),), native_key=NATIVE_KEY
    )
    second = StableBlockIdentity.from_intervals(
    ...
    index = MbaBlockIdentityIndex.from_bindings(
        session_id="session-a",
        generation=4,
        bindings=((first, 2), (second, 4)),
        native_key=NATIVE_KEY,
    )
    first_handle = index.handle_for_serial(2)
    second_handle = index.handle_for_serial(4)
    assert first_handle is not None and second_handle is not None
    attempt = TransactionAttemptId("plan-a", "attempt-rollback")
    ref = PlanBlockRef("plan-a", "created")
    index.begin_transaction(attempt.attempt_id, 5)
    reservation = index.reserve_plan_block(attempt, ref)
    index.bind_reserved_plan_block(
        attempt, ref, insertion_serial=2, returned_serial=3
    )

    index.discard_reserved_insert(
        transaction_id=attempt.attempt_id,
        handle=reservation.logical_version.handle,
    )

    assert index.resolve(first_handle, transaction_id=attempt.attempt_id).serial == 2
    assert index.resolve(second_handle, transaction_id=attempt.attempt_id).serial == 4


def test_anonymous_synthetic_factory_and_generic_reservation_cannot_claim_creation() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        generation=1, bindings=(), native_key=NATIVE_KEY
    )
    anonymous = index.create_synthetic_handle()
    assert anonymous.provenance is BlockHandleProvenance.OBSERVED_EPHEMERAL

    index.ensure_serial_space(1)
    source = index.handle_for_serial(0)
    assert source is not None
    index.begin_transaction("generic-reserve", 1)
    forged_created = MbaBlockHandle.created_synthetic(
        session_id=index.session_id,
        token="forged-created",
    )
    index._handles_by_token[forged_created.token] = forged_created
    with pytest.raises(ValueError, match="PlanBlockRef"):
        index.reserve_new_proxy(
            transaction_id="generic-reserve",
            handle=forged_created,
        )
    with pytest.raises(ValueError, match="PlanBlockRef"):
        index.record_clone(
            transaction_id="generic-reserve",
...
def test_missing_planned_serial_is_observed_ephemeral_not_created() -> None:
    index = MbaBlockIdentityIndex.from_bindings(
        generation=1, bindings=(), native_key=NATIVE_KEY
    )
    index.begin_transaction("realized", 0)
    index.record_realized_serial(
        transaction_id="realized",
        expected_serial=3,
        returned_serial=3,
    )

    handle = index._handles_by_token["planned:realized:3"]
    assert handle.provenance is BlockHandleProvenance.OBSERVED_EPHEMERAL
