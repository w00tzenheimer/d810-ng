...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 536
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("31b6db5bb61073da5d7cca7c936ea9ee72aab445")
    ...
    assert len(accepted_operation_ids) == 126
    assert accepted_operation_ids[-1] == "rhad:route@0x40B8B0"
    assert (
