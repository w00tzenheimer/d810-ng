...
def test_row151_inventory_owns_producer_and_both_target_fragments() -> None:
    ...
    assert row151["current_compiler_support"] == "unsupported_typed_shape"
    assert row151["current_generated_proof"] == {"status": "unproved"}
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert existing == {
        ...
        "vertically_proved_operations": 80,
        "accepted_receipt_operations": 80,
        "earliest_unproved_reference_order": 151,
        "earliest_unproved_operation_id": "rhad:route@0x40BCE3",
        "first_missing_typed_obligation": (
            "add typed existing-conditional compiler evidence for row151 before "
            "live mutation"
        ),
    }
    assert setcc == {
