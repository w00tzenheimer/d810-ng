...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 473
...
def test_stable_228_row_inventory_references_required_table_artifacts() -> None:
    ...
    assert row120["current_compiler_support"] == "unsupported_typed_shape"
    assert row120["current_generated_proof"] == {"status": "unproved"}
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("c74331cac2baa6c7dcd40c91079a2c95ea836b57")
    ...
    assert len(accepted_operation_ids) == 115
    assert accepted_operation_ids[-1] == "rhad:route@0x40B722"
    assert (
    ...
    assert existing == {
        ...
        "compiled_operation_instances": 59,
        "vertically_proved_operations": 59,
        "accepted_receipt_operations": 59,
        "earliest_unproved_reference_order": 120,
        "earliest_unproved_operation_id": "rhad:route@0x40B73C",
        "first_missing_typed_obligation": (
