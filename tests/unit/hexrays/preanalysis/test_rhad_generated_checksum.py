...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 854
...
def test_row203_inventory_has_exact_producer_and_complete_target_closure() -> None:
    ...
    assert row203["current_generated_proof"] == {"status": "unproved"}
...
def test_checksum_producer_compiles_row183_existing_dependency() -> None:
    ...
    assert batch.aggregate_program_identity == (
        "sha256:988c6d57591ecf0476d7fd8d9735dcda0dca088b6e512fb1613968fce4022741"
    )
    assert len(batch.operations) == 199
    assert len(batch.imported_blocks) == 854
    assert len(batch.template_fragments) == 187
    assert len(batch.native_body_proof_ids) == 199
    assert len(batch.native_body_entry_block_ids) == 548
    assert len(plan.blocks) == 859
    normalization = operation.computed_branch_normalization
    assert normalization is not None
    assert normalization.predicate_kind is PredicateKind.NE
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("1bafdf09b42e75c526973682525662059285bd18")
    ...
    assert len(accepted_operation_ids) == 198
    assert accepted_operation_ids[-1] == "rhad:route@0x40C4DA"
    assert (
    ...
    assert existing == {
        ...
        "vertically_proved_operations": 111,
        "accepted_receipt_operations": 111,
        "earliest_unproved_reference_order": 203,
        "earliest_unproved_operation_id": "rhad:route@0x40C4F4",
        "first_missing_typed_obligation": (
            "publish the compiled row203 operation and complete C4F6/C525 target closure "
            "at actual MMAT_GENERATED through the shared coordinator, then persist one "
            "committed receipt and maturity proof"
        ),
    }
    assert setcc == {
