...
def test_row155_inventory_owns_producer_and_both_target_fragments() -> None:
    ...
    assert row155["current_compiler_support"] == "unsupported_typed_shape"
    assert row155["current_generated_proof"] == {"status": "unproved"}
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert existing == {
        ...
        "compiled_operation_instances": 83,
        "vertically_proved_operations": 83,
        "accepted_receipt_operations": 83,
        "earliest_unproved_reference_order": 155,
        "earliest_unproved_operation_id": "rhad:route@0x40BD68",
        "first_missing_typed_obligation": (
            "add typed existing-conditional compiler evidence for row155 before "
            "live mutation"
        ),
    }
    assert setcc == {
