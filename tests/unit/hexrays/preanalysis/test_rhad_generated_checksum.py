...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 111
...
def test_checksum_producer_compiles_row23_existing_conditional_reference() -> None:
    ...
    assert payload["boundary_exit_eas"] == [0x40A607, 0x40B6C0]
    assert {"native@0x40A868", "native@0x40A8A7"}.issubset(
        plan.native_bodies[0].block_ids
    )
    true_template = next(
...
def test_indirect_jump_coverage_summary_matches_committed_row22_batch() -> None:
    summary = json.loads(
        (
            _REPO
            / "docs"
    ...
    assert summary["accepted_code_sha"] == ("5a4ce7138b3c956261d80c929f4da5ce6e573fc6")
    ...
    assert existing == {
        ...
        "earliest_unproved_reference_order": 23,
        "earliest_unproved_operation_id": "rhad:route@0x40A866",
