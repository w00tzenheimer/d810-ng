...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 559
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("17d0f5c8e1b24a1428a893aa5196a61a403d1ab9")
    ...
    assert len(accepted_operation_ids) == 129
    assert accepted_operation_ids[-1] == "route:rhad-direct@0x40B931"
    assert (
