...
def test_row183_inventory_has_producer_inclusive_exact_target_closures() -> None:
    ...
    assert row183["current_generated_proof"] == {"status": "unproved"}
...
def test_checksum_producer_compiles_row183_existing_dependency() -> None:
    ...
    assert batch.aggregate_program_identity == (
        "sha256:547cbec9edc5b6d0b58b750be9543705018d52b173f41f750637ca3ed3dbe86b"
    )
    assert len(batch.operations) == 179
    assert len(batch.imported_blocks) == 792
    assert len(batch.template_fragments) == 177
    assert len(batch.native_body_proof_ids) == 179
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("5ac733c9cbb7ebc1cbca236d97d674c942fff3ad")
    ...
    assert len(accepted_operation_ids) == 178
    assert accepted_operation_ids[-1] == "rhad:route@0x40C184"
    assert (
    ...
    assert existing == {
        ...
        "first_missing_typed_obligation": (
            ...
            "compiled row183 through the shared coordinator before advancing "
