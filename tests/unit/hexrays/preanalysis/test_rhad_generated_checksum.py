...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 282
...
def test_indirect_jump_coverage_summary_matches_committed_row54_batch() -> None:
    summary = json.loads(
        (
            _REPO
            / "docs"
    ...
    assert summary["accepted_code_sha"] == ("19745aac85f1937b94f67c7e4e83484d3a73f29c")
    ...
    assert existing == {
        ...
        "earliest_unproved_reference_order": 55,
        "earliest_unproved_operation_id": "rhad:route@0x40ACD7",
        "first_missing_typed_obligation": (
