...
def test_checksum_producer_compiles_row17_scaled_lookup_reference() -> None:
    ...
    assert len(IMPORTED_BLOCK_IDS) == 494
...
def test_stable_228_row_inventory_references_required_table_artifacts() -> None:
    ...
    assert row123["current_generated_proof"] == {"status": "unproved"}
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("fe70e3bb3482cd4af2e397739eaca64a3dc89edc")
    ...
    assert len(accepted_operation_ids) == 118
    assert accepted_operation_ids[-1] == "route:rhad-direct@0x40B781"
    assert (
    ...
    assert existing == {
        ...
        "compiled_operation_instances": 62,
        "vertically_proved_operations": 61,
        "accepted_receipt_operations": 61,
        "earliest_unproved_reference_order": 123,
        "earliest_unproved_operation_id": "rhad:route@0x40B7A8",
        "first_missing_typed_obligation": (
