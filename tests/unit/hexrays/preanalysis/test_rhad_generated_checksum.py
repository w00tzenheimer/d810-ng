...
def test_row187_inventory_has_producer_inclusive_exact_target_closures() -> None:
    ...
    assert row187["current_generated_proof"] == {"status": "unproved"}
...
def test_checksum_producer_compiles_row183_existing_dependency() -> None:
    ...
    assert batch.aggregate_program_identity == (
        "sha256:323aa30244d67791bb95cf92d13b6fbc427c817ce3381ae49d9fd86e81b6b6c0"
    )
    assert len(batch.operations) == 183
    assert len(batch.imported_blocks) == 808
    assert len(batch.template_fragments) == 179
    assert len(batch.native_body_proof_ids) == 183
...
def test_indirect_jump_coverage_summary_matches_committed_acceptance_prefix() -> None:
    ...
    assert summary["accepted_code_sha"] == ("2ee0eb4138e1f894a2d4e91287e703d95e32fd82")
    ...
    assert len(accepted_operation_ids) == 182
    assert accepted_operation_ids[-1] == "route:rhad-direct@0x40C251"
    assert (
    ...
    assert existing == {
        ...
        "vertically_proved_operations": 102,
        "accepted_receipt_operations": 102,
        "earliest_unproved_reference_order": 187,
        "earliest_unproved_operation_id": "rhad:route@0x40C26B",
        "first_missing_typed_obligation": (
            "bind the compiled row187 RhadExistingConditionalRoute through "
            "immutable preflight and actual GENERATED publication before live "
            "mutation"
        ),
    }
    assert setcc == {
