...
def test_rewrite_import_text_handles_moved_modules_and_symbols() -> None:
    ...
    source = "\n".join(
        [
            "import d810.cfg.region_oracle_cli as cli",
            "from d810.recon.flow.edge_diagnostics import classify_dag_edges",
            "from d810.cfg.ref_region_oracle import BlockView, collect_block_views_for_snapshot",
    ...
    assert "import d810.diagnostics.region_oracle_cli as cli" in result.text
    assert (
        "from d810.diagnostics.edge_diagnostics import classify_dag_edges"
        in result.text
    )
    assert (
        "from d810.cfg.ref_region_oracle import BlockView"
        in result.text
    )
    assert (
