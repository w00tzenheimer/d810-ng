...
def build_effective_maturity_schedule(
    ...
    *,
    ...
    allow_legacy_constant_fallback: bool = False,
) -> EffectiveMaturitySchedule:
    ...
    fallback for other passes until their contracts are migrated.  The
    constant bundle may use live rules only when the caller explicitly opts
    into legacy fallback mode.
    """
    ...
    for configured_index, config in enumerate(configs):
        ...
        if is_constant_bundle:
            if constant_simplification_schedule is None:
                if allow_legacy_constant_fallback:
                    # Legacy projects have no immutable config-v2 schedule.
                    # Their explicit opt-in preserves the pre-Task-4 view.
                    pass
                else:
                    for descriptor in descriptors:
                        ...
                        if not isinstance(descriptor_pipeline, ExecutionPipeline):
                            continue
                        lifecycle = getattr(descriptor, "lifecycle_domain", None)
                        projected.append(
                            EffectiveScheduleStage(
                                configured_index=configured_index,
                                runtime_order=-1,
                                pass_id=str(
                                    getattr(descriptor, "pass_id", config.pass_id)
                                ),
                                stage_id=str(getattr(descriptor, "stage_id")),
                                pipeline=descriptor_pipeline.value,
                                implementation_name=str(
                                    getattr(descriptor, "implementation_name", "")
                                ),
                                requirements=requirements,
                                provider_maturities=(),
                                maturity_source="compiled stage contract unavailable",
                                enabled=False,
                                supported_maturities=tuple(
                                    str(getattr(maturity, "name", maturity))
                                    for maturity in getattr(
                                        descriptor, "supported_maturities", ()
                                    )
                                ),
                                ...
                                lifecycle_domain=str(
                                    getattr(lifecycle, "name", lifecycle)
                                ),
                    ...
                    projected.append(
                        EffectiveScheduleStage(
                            ...
                            runtime_order=-1,
                            pass_id=str(
                                getattr(config, "pass_id", "constant-simplification")
                            ),
                            ...
                            preparation_reason="compiled schedule unavailable",
                        )
                    )
                    continue
            if constant_simplification_schedule is None:
                # Explicit legacy fallback continues through the old live-rule
                # projection below.  No compiled-schedule fields are invented.
                pass
            else:
                ...
                for compiled_stage in compiled_stages:
                    ...
                    projected.append(
                        EffectiveScheduleStage(
                            ...
                            pass_id=str(
                                getattr(compiled_stage, "pass_id", config.pass_id)
                            ),
                            ...
                            inactive_reason=getattr(
                                compiled_stage, "inactive_reason", None
                            ),
                        )
                    )
                ...
                if preparation is not None:
                    ...
                    unknown_items = tuple(getattr(preparation_status, "unknown", ()) or ())
                    unknown = max(
                        int(getattr(preparation_status, "unknown_count", 0) or 0),
                        len(unknown_items),
                    )
                    failures = tuple(
                        sorted(
                            {
                                ": ".join(
                                    ...
                                    for name in ("provider", "error_type", "message")
                                )
                    ...
                    preparation_state = ", ".join(bucket_states) or (
                        "ready"
                    ...
                    projected.append(
                        EffectiveScheduleStage(
                            ...
                            preparation_reason=(
                                "next preparation round" if pending else None
                            ),
                            preparation_pending_count=pending,
                            preparation_applied_count=applied,
                            preparation_conflicting_count=conflicting,
                            preparation_restored_count=restored,
                            preparation_unknown_count=unknown,
                            preparation_provider_failures=failures,
                        )
                    )
                continue
        for descriptor in descriptors:
    ...
    for ordinal, maturity in enumerate(IR_MATURITY_ORDER):
        ...
        by_pipeline: dict[str, list[EffectiveScheduleStage]] = {}
        for stage in stages:
            if provider in stage.provider_maturities:
                by_pipeline.setdefault(stage.pipeline, []).append(stage)
        pipeline_stages = tuple(
            ...
            for pipeline, pipeline_values in sorted(by_pipeline.items())
        )
        rows.append(
            EffectiveMaturityScheduleRow(
                ordinal=ordinal,
