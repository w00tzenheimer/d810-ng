...
def plan_lower_back(function: LlvmLowerBackFunction) -> LlvmLowerBackResult:
    ...
    for block in function.blocks:
        for phi in block.phis:
            _plan_phi(function, block, phi, blocks, edge_moves, unsupported)
    ...
    return LlvmLowerBackResult(
        ...
        plan=LlvmLowerBackPlan(
            ...
            block_order=tuple(block.label for block in function.blocks),
...
def _plan_phi(
    ...
    for incoming in phi.incoming:
        ...
        if _requires_critical_edge_split(pred, block):
            unsupported.append(
                LlvmLowerBackUnsupportedReason(
                    kind=LlvmLowerBackUnsupportedKind.CRITICAL_EDGE_SPLIT_REQUIRED,
                    block_label=block.label,
                    operation="phi",
                    reason=(
                        f"edge {pred.label!r}->{block.label!r} needs a split "
                        "before placing the PHI edge move"
                    ),
                )
            )
            continue
