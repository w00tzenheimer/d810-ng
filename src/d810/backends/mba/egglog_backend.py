...
def requires_egglog(func: typing.Callable[..., typing.Any]):
...
class BitExpr(egglog.Expr):
    ...
    @classmethod
    def var(cls, name: egglog.StringLike) -> "BitExpr":
...
class MBAEGraph:
    ...
    def _setup_egraph(self):
        ...
        x, y, z = egglog.vars_("x y z", BitExpr)
        ...
        self.egraph.register(
            # x ^ x = 0
            egglog.rewrite(x ^ x).to(zero),
            # x & 0 = 0
            egglog.rewrite(x & zero).to(zero),
            # x * 0 = 0
            egglog.rewrite(x * zero).to(zero),
            # x - x = 0
            egglog.rewrite(x - x).to(zero),
        ...
        self.egraph.register(
            ...
            egglog.rewrite((x | y) - (x & y)).to(x ^ y),
            # Xor_HackersDelightRule_2: 2*(x | y) - (x + y) => x ^ y
            egglog.rewrite(two * (x | y) - (x + y)).to(x ^ y),
            # Xor_HackersDelightRule_3: (x + y) - 2*(x & y) => x ^ y
            egglog.rewrite((x + y) - two * (x & y)).to(x ^ y),
            # Xor_HackersDelightRule_5: x - (2*(x & y) - y) => x ^ y
            egglog.rewrite(x - (two * (x & y) - y)).to(x ^ y),
        ...
        self.egraph.register(
            ...
            egglog.rewrite((x & y) | (x ^ y)).to(x | y),
            # Or_Rule_2: (x ^ y) | y => x | y
            egglog.rewrite((x ^ y) | y).to(x | y),
            # Or_Rule_4: (x & y) ^ (x ^ y) => x | y
            egglog.rewrite((x & y) ^ (x ^ y)).to(x | y),
            # OrBnot_FactorRule_1: ~x ^ (x & y) => ~x | y
            egglog.rewrite((~x) ^ (x & y)).to((~x) | y),
        ...
        self.egraph.register(
            ...
            egglog.rewrite((x - one) - (two * x)).to(~x),
            # Bnot_FactorRule_1: ~(x ^ y) ^ y => ~x
            egglog.rewrite((~(x ^ y)) ^ y).to(~x),
            # Bnot_FactorRule_4: ~x ^ ~y => x ^ y
            egglog.rewrite((~x) ^ (~y)).to(x ^ y),
        )
        ...
        self.egraph.register(
            ...
            egglog.rewrite(x & (~x)).to(zero),
            # Pred0Rule5: x & ~(x | y) => 0
            egglog.rewrite(x & (~(x | y))).to(zero),
            # Pred0Rule6: (x & y) & ~(x | y) => 0
            egglog.rewrite((x & y) & (~(x | y))).to(zero),
        ...
        self.egraph.register(
            ...
            egglog.rewrite((x & z) | (y & z)).to((x | y) & z),
        ...
        self.egraph.register(
            ...
            egglog.rewrite(two * x + (~x)).to(x - one),
            # Sub1Or_MbaRule_1: (x + y) + ~(x & y) => (x | y) - 1
            egglog.rewrite((x + y) + (~(x & y))).to((x | y) - one),
        )
        ...
        self.egraph.register(
            ...
            egglog.rewrite((~(x ^ y)) + two * (y | x)).to((x + y) - one),
        )
    ...
    def saturate(self):
        ...
        for i in range(self.max_iterations):
            try:
                ...
                if (
                    hasattr(report, "stop_reason")
                    and str(report.stop_reason) == "Saturated"
                ):
                    break
    ...
    def check_equivalent(self, expr1: BitExpr, expr2: BitExpr) -> bool:
        ...
        try:
            self.egraph.check(egglog.eq(expr1).to(expr2))
            return True
        except Exception:
            return False
...
class AstToBitExprConverter:
    ...
    def convert(self, ast_node) -> BitExpr | None:
        ...
        if (
            isinstance(ast_node, (AstLeafProtocol, AstConstantProtocol))
            or ast_node.is_leaf()
        ):
...
class EGraphOptimizer:
    ...
    def try_simplify(self, ast_node) -> BitExpr | None:
    ...
    def _find_simplification(
        ...
        candidates = [
            (x | y, idaapi.m_or, "OR"),
            (x & y, idaapi.m_and, "AND"),
            (x ^ y, idaapi.m_xor, "XOR"),
            (x, None, "X"),
            (y, None, "Y"),
        ]
...
def _create_pattern_egraph() -> egglog.EGraph:
