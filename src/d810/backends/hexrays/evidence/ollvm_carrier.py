...
from d810.ir.expressions import (
    Add,
    And,
    Const,
    ExprRef,
...
class _OllvmCarrierInsn:
    ...
    # Store-target stack offset (``instruction.result`` when it is a stack
    # slot), read by the structural accumulator detection together with the
    # lifted stored-value expression tree.  ``None`` when the destination is
    # not a stack slot.
    dest_stkoff: int | None = None
    # Lifted operand expression trees (slot-indexed), evidence-only.  Populated
    # from ``Instruction.input_exprs[0/1]``; ``input_exprs[0]`` is the LEFT
    # operand (the STORED VALUE subtree for an ``m_stx`` STORE).  ``None`` when
    # the projection could not lift that operand.
    ...
    def from_canonical(
        ...
        *,
        ...
        ea_raw = attrs.get("ea")
        input_exprs = instruction.input_exprs
        # The store-target stack offset comes from ``_canonical_operands`` (which
        # routes a STORE through ``_store_operands``), NOT ``instruction.result``
        # -- an ``m_stx`` STORE has no register result, so its target lives in
        # the memory/effect target instead.
        dest, _left, _right = _canonical_operands(instruction)
        return cls(
            ...
            src_l_expr=input_exprs[0] if len(input_exprs) > 0 else None,
            src_r_expr=input_exprs[1] if len(input_exprs) > 1 else None,
        )
...
# The ``ACCUMULATOR_CARRIER`` self-update role is detected by walking the lifted
# stored-value expression tree (``_OllvmCarrierInsn.src_l_expr``) when it is
# available (the production projection branch), falling back to the legacy
# ``dstr`` text regex when it is not (the diag/legacy projection branch, which
# leaves ``src_l_expr`` ``None``).  These structural constants mirror the legacy
# text markers: ``#5.4*`` multiply, ``+#0x42`` / ``-#0x42`` add/sub, and the
# ``#0xFFFFFFBD`` mask.
_ACC_MUL_CONST = 5
_ACC_ADD_CONST = 0x42
_ACC_MASK_CONST = 0xFFFFFFBD


def _render_stack_token(stkoff: int) -> str:
    """Render a stack offset as the canonical ``%var_NN`` carrier token.

    Hex-Rays names a stack lvar ``%var_<offset-in-uppercase-hex>`` and the legacy
    text path extracted exactly that string from ``dstr``.  The structural path
    reproduces it from the operand's stack offset so no ``dstr`` parsing is
    needed for the accumulator role.  ``_canonical_token`` is applied for parity
    with the text path (it already upper-cases, but keep the single source of
    token normalization).
    """
    return _canonical_token(f"%var_{int(stkoff):X}") or f"%var_{int(stkoff):X}"


@dataclass(frozen=True)
class _ValueExprSignature:
    """Structural summary of a stored-value ``ExprRef`` subtree.

    ``stack_offsets`` are the stack-leaf offsets in deterministic, de-duplicated
    order (the structural analogue of ``_tokens(dstr)[:-1]``).  ``const_ops`` are
    ``(enclosing-operation, value)`` pairs (the analogue of the ``#5.4*`` /
    ``+#0x42`` / ``#0xFFFFFFBD`` marker substring checks).
    """

    stack_offsets: tuple[int, ...]
    const_ops: frozenset[tuple[ValueOpKind | None, int]]

    @property
    def consts(self) -> frozenset[int]:
        return frozenset(value for _op, value in self.const_ops)

    def has_const_under(
        self, value: int, *, ops: frozenset[ValueOpKind] | None = None
    ) -> bool:
        for op, const in self.const_ops:
            if const != value:
                continue
...
def _expr_stack_offset(expr: ExprRef | None) -> int | None:
    """Return the stack offset a ``Move`` leaf refers to, else ``None``."""
...
def _value_expr_signature(expr: ExprRef | None) -> _ValueExprSignature:
    offsets: list[int] = []
    const_ops: set[tuple[ValueOpKind | None, int]] = set()
    _walk_value_expr(expr, None, offsets, const_ops)
    return _ValueExprSignature(
        stack_offsets=tuple(dict.fromkeys(offsets)),
        const_ops=frozenset(const_ops),
    )


def _walk_value_expr(
    expr: ExprRef | None,
    parent_op: ValueOpKind | None,
    offsets: list[int],
    const_ops: set[tuple[ValueOpKind | None, int]],
) -> None:
    if expr is None:
        return
    if isinstance(expr, Const):
        const_ops.add((parent_op, int(expr.value)))
        return
    stkoff = _expr_stack_offset(expr)
    if stkoff is not None:
        offsets.append(stkoff)
        return
    node_op = _EXPR_NODE_OPS.get(type(expr))
    children = ()
    if isinstance(expr, (Add, Sub, And, Mul)):
        children = (expr.left, expr.right)
    for child in children:
        _walk_value_expr(child, node_op, offsets, const_ops)


_EXPR_NODE_OPS: dict[type, ValueOpKind] = {
    Add: ValueOpKind.ADD,
    Sub: ValueOpKind.SUB,
    And: ValueOpKind.AND,
    Mul: ValueOpKind.MUL,
...
def _structural_accumulator_hit(
    insn: _OllvmCarrierInsn,
    local_pointer_base: dict[str, str],
) -> _CarrierHit | None:
    """Structural ``ACCUMULATOR_CARRIER`` self-update detection.

    Reads the lifted stored-value expression tree (``insn.src_l_expr``; the store
    value is ``insn.l``) and the store-target stack offset (``insn.dest_stkoff``)
    instead of the rendered ``dstr``.  The emitted payload is byte-identical to
    the legacy text path: ``store_kind``, and (for the ``#5*`` multiply-add
    shape) ``multiply_add_operand_tokens`` / ``multiply_add_base_token`` /
    ``multiply_add_same_base_alias_tokens`` / ``same_carrier_alias_proof``.

    ``local_pointer_base`` stays the (unchanged) regex-derived alias map, so the
    base/alias details match the text path exactly.
    """
    target_off = insn.dest_stkoff
    if target_off is None:
        return None
    signature = _value_expr_signature(insn.src_l_expr)
    if target_off not in signature.stack_offsets:
        return None

    has_mul5 = signature.has_const_under(
        _ACC_MUL_CONST, ops=frozenset({ValueOpKind.MUL})
    )
    has_add42 = signature.has_const_under(
        _ACC_ADD_CONST, ops=frozenset({ValueOpKind.ADD, ValueOpKind.SUB})
    )
    has_mask = _ACC_MASK_CONST in signature.consts
    if not (has_mul5 or has_add42 or has_mask):
        return None
...
def _iter_carrier_hits(instructions: tuple[_OllvmCarrierInsn, ...]) -> Iterable[_CarrierHit]:
    ...
    for insn in instructions:
        ...
        if insn.operation is ValueOpKind.STORE:
            ...
            if target is None:
                continue
            # Structural-primary, regex-FALLBACK, single-emit: when the lifted
            # stored-value expression tree is a structured operand subtree (the
            # production projection branch yields ``Add``/``Sub``/``And``/``Mul``
            # nodes), detect the self-update accumulator structurally; otherwise
            # fall back to the legacy ``dstr`` regex.  The fallback covers the
            # diag/legacy branch -- where ``src_l_expr`` is ``None`` or only a
            # bare ``Const``/``Move`` leaf, because a partial ``meta`` tree never
