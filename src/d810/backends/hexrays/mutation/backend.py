...
class HexRaysMutationBackend:
    ...
    def publish_fragment(
        ...
        """Compatibility protocol method; all mutation routes through ``apply``."""
        if not isinstance(fragment_plan, FragmentPlan):
            raise TypeError("publish_fragment requires FragmentPlan")
        return self.apply(fragment_plan, live_source, safety_policy)
