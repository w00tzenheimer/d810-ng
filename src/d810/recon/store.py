"""Migration shim: ``d810.recon.store`` -> ``d810.passes.store`` (dissolution, llr-lyly).

sys.modules alias preserving the old import path; re-exports public AND
private symbols.  Deleted in Phase Z once consumers repoint.

The store module is a *passes*-layer module (recon orchestration root /
scheduler tier, driven by Manager above the analyses layer), so the
canonical home is :mod:`d810.passes.store`.  ``d810.recon`` sits BELOW
...
edge.  All live importers repoint to ``d810.passes.store`` directly.
"""
import importlib
import sys

_canonical = importlib.import_module("d810.passes.store")
