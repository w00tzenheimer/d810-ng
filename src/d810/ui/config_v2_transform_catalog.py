"""Fixed Qt renderer for pass-owned config-v2 transform catalogs.

The pass registration owns the catalog metadata.  This widget only renders the
closed projection and reports intentional operator actions to its panel.
"""
...
except ImportError:  # Narrow fake-Qt test adapters may omit the availability flag.
...
if QT_GRAPHICS_AVAILABLE:
    ...
    class ConfigV2TransformCatalogWidget(QtWidgets.QWidget):
        """Filterable fixed tree of explicit family/subfamily metadata."""
        ...
        def __init__(
            ...
            *,
            ...
            self._catalog: ConfigV2TransformCatalogView | None = None
            self._by_target: dict[str, ConfigV2TransformView] = {}

            self.query = QtWidgets.QLineEdit(self)
            self.query.setPlaceholderText(
                "Filter transforms, families, verification, or advisory..."
            )
            self.tree = QtWidgets.QTreeWidget(self)
            self.tree.setColumnCount(2)
            self.tree.setHeaderLabels(["Transform family", "Selection"])
            ...
            self.overflow_menu.addAction("Select every transform", self._select_all)
            self.overflow_menu.addAction("Clear every transform", self._clear_all)
        ...
        def set_catalog(self, catalog: ConfigV2TransformCatalogView | None) -> None:
            """Render an explicit catalog view without changing its selection."""
                ...
                for family in catalog.families:
                    ...
                    for subfamily in family.subfamilies:
                        ...
                        for transform in subfamily.transforms:
                            ...
                            if transform.advisory != "none":
                                label += f" [{_display_status(transform.advisory)}]"
                            leaf = self._item(
                                label,
                                transform.transform_id,
                                "checked" if transform.selected else "unchecked",
                            )
                            subfamily_item.addChild(leaf)
                            self._by_target[transform.transform_id] = transform
                self.tree.expandAll()
                self._render_empty()
            finally:
                self._rendering = False

        def current_catalog(self) -> ConfigV2TransformCatalogView | None:
        ...
        def _item(
            ...
            flags = qt_flag_or(item.flags(), _checkable_flag())
            item.setFlags(flags)
        ...
        def _current_item_changed(self, item: typing.Any, previous: typing.Any) -> None:
            ...
            transform = self._by_target.get(target_id)
            if transform is None:
                self._render_empty()
                return
            self.details.set_sections(self._details_for(transform))

        def _details_for(
            self,
            transform: ConfigV2TransformView,
        ) -> tuple[DetailSection, ...]:
            fields = [
                DetailField("Transform", f"{transform.label} ({transform.transform_id})"),
                DetailField("Description", transform.description),
                DetailField("Reference", transform.reference),
                DetailField("Maturities", ", ".join(transform.maturities)),
                DetailField(
                    "Verification",
                    f"{_display_status(transform.verification)}: {transform.verification_reason}",
                ),
            ]
            if transform.advisory != "none":
                fields.append(
                    DetailField(
                        "Advisory",
                        f"{_display_status(transform.advisory)}: {transform.advisory_reason}",
                    )
                )
            if transform.cost != "unknown":
                fields.append(
                    DetailField(
                        "Cost",
                        f"{_display_status(transform.cost)}: {transform.cost_detail}",
                    )
                )
            return (DetailSection("transform", "Transform metadata", tuple(fields)),)

        def _render_empty(self) -> None:
            if self._catalog is None:
                message = "This pass has no individually selectable transforms."
            elif not self._catalog.visible_transform_ids:
                message = "No transforms match the current filter."
            else:
                message = "Select a transform to inspect its metadata."
            self.details.set_sections(
                (DetailSection("selection", "Transform metadata", (DetailField("Selection", message),)),)
            )
    ...
    class ConfigV2TransformCatalogWidget:
        def __init__(self, *args: typing.Any, **kwargs: typing.Any) -> None:
            del args, kwargs
            raise ImportError("ConfigV2TransformCatalogWidget requires IDA Pro Qt")


__all__ = ["ConfigV2TransformCatalogWidget"]
