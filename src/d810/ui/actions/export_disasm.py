"""Export disassembly to various formats (ASM/LST/MAP/IDC).

Export the current function's disassembly using IDA's native exporters.
Available from disassembly view.
"""
from __future__ import annotations

from pathlib import Path

from d810.core import typing

from d810.core.logging import getLogger
from d810.ui.actions.base import D810ActionHandler
from d810.ui.actions.export_disasm_logic import (
    DisasmExportSettings,
    to_ida_flags_with_loader,
    to_ida_format_int_with_loader,
)
from d810.ui.export_disasm_masm_emit import (
    IDA_AVAILABLE as MASM_EMIT_AVAILABLE,
    generate_masm_for_function,
)

logger = getLogger("D810.ui")

# ---------------------------------------------------------------------------
# Qt imports -- optional, will fail gracefully if not in GUI mode
# ---------------------------------------------------------------------------
try:
    from d810.qt_shim import (
        QDialog,
        QVBoxLayout,
        QHBoxLayout,
        QLabel,
        QComboBox,
        QCheckBox,
        QPushButton,
        QLineEdit,
    )

    QT_AVAILABLE = True
except ImportError:
    QT_AVAILABLE = False


class ExportDisassemblyDialog(QDialog if QT_AVAILABLE else object):  # type: ignore[misc]
    """Dialog for configuring disassembly export options."""

    def __init__(
        self,
        func_name: str,
        parent=None,
        idaapi_shim: typing.Any | None = None,
    ):
        """Initialize the export disassembly dialog.

        Args:
            func_name: Name of the function being exported (for default filename)
            parent: Parent widget
        """
        super().__init__(parent)
        self.func_name = func_name
        self._idaapi_shim = idaapi_shim
        self.setWindowTitle("Export Disassembly")
        self.setup_ui()

    def setup_ui(self):
        """Set up the dialog UI."""
        layout = QVBoxLayout()

        # Format selection
        format_layout = QHBoxLayout()
        format_layout.addWidget(QLabel("Format:"))
        self.format_combo = QComboBox()
        self.format_combo.addItems(["ASM", "LST", "MAP", "IDC", "MASM"])
        format_layout.addWidget(self.format_combo)
        layout.addLayout(format_layout)

        # Options checkboxes
        self.include_headers_check = QCheckBox("Include segment headers")
        self.include_headers_check.setChecked(True)
        layout.addWidget(self.include_headers_check)

        # MASM-only: place materialized data in a read-only CONST (.rdata)
        # segment instead of the writable _DATA segment.
        self.const_data_check = QCheckBox("MASM: read-only data (CONST segment)")
        self.const_data_check.setChecked(False)
        layout.addWidget(self.const_data_check)

        # Output file selection
        file_layout = QHBoxLayout()
        file_layout.addWidget(QLabel("Output file:"))
        self.file_edit = QLineEdit()
        self.file_edit.setText(f"{self.func_name}.asm")
        file_layout.addWidget(self.file_edit)

        browse_btn = QPushButton("Browse...")
        browse_btn.clicked.connect(self.browse_file)
        file_layout.addWidget(browse_btn)
        layout.addLayout(file_layout)

        # Buttons
        button_layout = QHBoxLayout()
        ok_btn = QPushButton("OK")
        ok_btn.clicked.connect(self.accept)
        cancel_btn = QPushButton("Cancel")
        cancel_btn.clicked.connect(self.reject)
        button_layout.addWidget(ok_btn)
        button_layout.addWidget(cancel_btn)
        layout.addLayout(button_layout)

        self.setLayout(layout)

    def browse_file(self):
        """Show file browser dialog."""
        if self._idaapi_shim is None:
            return

        file_path = self._idaapi_shim.ask_file(
            1, self.file_edit.text(), "Save disassembly as..."
        )
        if file_path:
            self.file_edit.setText(file_path)

    def get_settings(self) -> dict:
        """Get the configured export settings.

        Returns:
            Dictionary with format, include_headers, output_path
        """
        format_str = self.format_combo.currentText()
        return {
            "format": format_str,
            "include_headers": self.include_headers_check.isChecked(),
            "const_data": self.const_data_check.isChecked(),
            "output_path": self.file_edit.text(),
        }


class ExportDisassembly(D810ActionHandler):
    """Export the current function's disassembly to various formats."""

    ACTION_ID = "d810ng:export_disasm"
    ACTION_TEXT = "Export disassembly..."
    ACTION_TOOLTIP = "Export function disassembly as ASM/LST/MAP/IDC"
    SUPPORTED_VIEWS = frozenset({"disasm"})
    MENU_ORDER = 71
    REQUIRES_STARTED = False

    def execute(self, ctx: typing.Any) -> int:
        """Execute the export disassembly action.

        Args:
            ctx: IDA action context

        Returns:
            1 on success, 0 on failure
        """
        idaapi_shim = self.ida_module("idaapi")
        if not QT_AVAILABLE or idaapi_shim is None:
            return 0

        # Get current function EA
        ea = idaapi_shim.get_screen_ea()
        func = idaapi_shim.get_func(ea)
        if func is None:
            logger.warning("ExportDisassembly: no function at cursor")
            idaapi_shim.warning("No function at cursor")
            return 0

        func_name = idaapi_shim.get_func_name(func.start_ea)
        if not func_name:
            func_name = f"sub_{func.start_ea:X}"

        # Show dialog
        dialog = ExportDisassemblyDialog(
            func_name,
            idaapi_shim=idaapi_shim,
        )
        if dialog.exec_() != QDialog.Accepted:
            logger.info("Export disassembly cancelled by user")
            return 0

        settings = dialog.get_settings()

        logic_settings = DisasmExportSettings(
            format=settings["format"],
            include_headers=settings["include_headers"],
            include_segments=False,  # Dialog currently exposes header toggle only
            output_path=settings["output_path"],
        )
        format_type = to_ida_format_int_with_loader(
            logic_settings.format, loader=idaapi_shim
        )
        flags = to_ida_flags_with_loader(logic_settings, loader=idaapi_shim)

        output_path = settings["output_path"]
        if not output_path:
            idaapi_shim.warning("No output file specified")
            return 0

        # MASM is generated by post-processing IDA's OFILE_ASM listing into a
        # compilable ml64 source with materialized data references.
        if logic_settings.format == "MASM":
            return self._export_masm(
                idaapi_shim, func, output_path,
                const_data=bool(settings.get("const_data", False)),
            )

        try:
            # gen_file() requires a real C FILE* (the SWIG-wrapped type returned
            # by ida_diskio.fopen*), NOT a qfile_t -- passing a qfile_t raises
            # "argument 2 of type 'FILE *'". Mirror idc.gen_file: write-binary
            # for EXE, write-text for the ASM/LST/MAP/IDC formats this dialog
            # offers, and close with ida_fpro.qfclose.
            ofile_exe = getattr(idaapi_shim, "OFILE_EXE", None)
            fopen = (
                idaapi_shim.fopenWB
                if ofile_exe is not None and format_type == ofile_exe
                else idaapi_shim.fopenWT
            )
            fp = fopen(output_path)
            if not fp:
                logger.error("Failed to open output file: %s", output_path)
                idaapi_shim.warning(f"Failed to open output file:\n{output_path}")
                return 0

            # gen_file returns the number of generated lines, or -1 on error.
            try:
                lines = idaapi_shim.gen_file(
                    format_type,
                    fp,
                    func.start_ea,
                    func.end_ea,
                    flags,
                )
            finally:
                idaapi_shim.qfclose(fp)

            if lines is not None and lines > 0:
                logger.info("Exported disassembly to %s", output_path)
                idaapi_shim.info(f"Disassembly exported to:\n{output_path}")
                return 1
            else:
                logger.error("Failed to export disassembly")
                idaapi_shim.warning("Failed to export disassembly")
                return 0

        except Exception as exc:
            logger.error("Failed to export disassembly: %s", exc)
            idaapi_shim.warning(f"Failed to export disassembly:\n{exc}")
            return 0

    def _export_masm(
        self,
        idaapi_shim: typing.Any,
        func: typing.Any,
        output_path: str,
        const_data: bool = False,
    ) -> int:
        """Export compilable ml64 MASM for the function via the structural generator.

        Decodes insn/op_t, resolves references to relocatable symbols, and
        materializes the referenced data closure (real constant values) so the
        rebuilt binary preserves the genuine code + data. When ``const_data`` is
        set, that data goes in a read-only ``CONST`` (.rdata) segment.
        """
        if not MASM_EMIT_AVAILABLE:
            logger.error("MASM export requires IDA (structural generator unavailable)")
            idaapi_shim.warning("MASM export requires IDA Pro")
            return 0
        try:
            source = generate_masm_for_function(func.start_ea, const_data=const_data)
            Path(output_path).write_text(source, encoding="utf-8")
            logger.info("Exported MASM to %s", output_path)
            idaapi_shim.info(f"MASM exported to:\n{output_path}")
            return 1
        except Exception as exc:
            logger.error("Failed to export MASM: %s", exc)
            idaapi_shim.warning(f"Failed to export MASM:\n{exc}")
            return 0

    def is_available(self, ctx: typing.Any) -> bool:
        """Check if action is available in current context.

        Args:
            ctx: IDA action context

        Returns:
            True if in disassembly view with a function at cursor
        """
        idaapi_shim = self.ida_module("idaapi")
        if idaapi_shim is None:
            return False

        # Check if we're in disassembly view
        widget_type = idaapi_shim.get_widget_type(ctx.widget)
        if widget_type != idaapi_shim.BWN_DISASM:
            return False

        # Check if there's a function at cursor
        ea = idaapi_shim.get_screen_ea()
        func = idaapi_shim.get_func(ea)
        return func is not None
