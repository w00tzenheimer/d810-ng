...
JUMP_FIXER_RULE_SPECS: tuple[RuleEditorSpec, ...] = (
    _jump_rule(
        ...
        family_id="comparison-rewrites",
        family_label="Comparison rewrites",
        subfamily_id="masked-constants",
        subfamily_label="Masked constants",
    ),
    _jump_rule(
    ...
    _jump_rule(
        "JaeRule1",
        "Unsigned above-or-equal identity",
        "Collapse an unsigned above-or-equal jump through its known identity.",
        family_id="comparison-rewrites",
        family_label="Comparison rewrites",
        subfamily_id="unsigned-identities",
        subfamily_label="Unsigned identities",
    ),
    _jump_rule(
        "JbRule1",
        "Unsigned below identity",
    ...
    _jump_rule(
        "JmpRuleAffineEq",
        "Affine branch equality",
        "Fold a jz/jnz predicate whose two sides have identical affine terms.",
        family_id="constant-recovery",
        family_label="Constant recovery",
        subfamily_id="affine-identities",
        subfamily_label="Affine identities",
    ),
    _jump_rule(
        ...
        subfamily_label="Flags register",
    ...
    _jump_rule(
        ...
        "Reaching-constant branch",
        "Fold a conditional branch when its operands are constant on the incoming path.",
        family_id="constant-recovery",
        family_label="Constant recovery",
        subfamily_id="path-constants",
        subfamily_label="Path constants",
    ),
    _jump_rule(
        "JmpRuleZ3Const",
        "SMT constant branch",
        ...
        family_id="constant-recovery",
        family_label="Constant recovery",
        subfamily_id="symbolic-proof",
        subfamily_label="Symbolic proof",
    ...
    *(
        _jump_rule(
            f"JnzRule{number}",
            f"Boolean opaque predicate {number}",
            "Collapse a recognized jz/jnz boolean opaque-predicate identity.",
            family_id="opaque-predicates",
            family_label="Opaque predicates",
            subfamily_id="boolean-identities",
            subfamily_label="Boolean identities",
        )
        for number in range(1, 9)
    ),
    _jump_rule(
        "JnzRuleModIdentity",
        "Modulo parity opaque predicate",
        "Recognize x * (x + 1) modulo 2 as an always-even predicate.",
        family_id="opaque-predicates",
        family_label="Opaque predicates",
        subfamily_id="modulo-parity",
        subfamily_label="Modulo parity",
    ),
    _jump_rule(
        "JnzRuleSmodSubIdentity",
        "Signed modulo subtraction predicate",
    ...
    _jump_rule(
        ...
        "Unsigned modulo addition predicate",
    ...
    _jump_rule(
        ...
        "Unsigned modulo subtraction predicate",
        "Recognize x * (x - 1) unsigned modulo 2 as an always-even predicate.",
        family_id="opaque-predicates",
        family_label="Opaque predicates",
        subfamily_id="modulo-parity",
...
_EDITOR_FIELDS_BY_PASS_ID: Mapping[str, tuple[FieldEditorSpec, ...]] = {
    ...
    JUMP_FIXER_PASS_ID: (
        ...
        FieldEditorSpec(
            ...
            label="Preserve discarded side effects",
            path=("preserve_z3_discarded_side_effects",),
            control=FieldControlKind.BOOLEAN,
            description=(
                "Keep side-effectful corridors discarded by a jump-folding rule."
            ),
            default=False,
            advisory=AdvisoryTone.WARNING,
            advisory_reason=(
        ...
        FieldEditorSpec(
            ...
            control=FieldControlKind.INTEGER,
            description="Maximum depth inspected for discarded side effects.",
            default=3,
            minimum=0,
            maximum=64,
        ),
