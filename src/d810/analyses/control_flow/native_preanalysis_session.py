...
class NativePreanalysisSessionState:
    ...
    def has_exhausted_poison_restart(self) -> bool:
        """Whether another poison incident exceeded this evidence epoch's retry."""
        return self.exhausted_poison_restart_generation == self.evidence_generation

    def request_generated_restart(
        self,
        *,
        reason: str = "CALLS staged a controller-owned generated-MBA restart",
        evidence_family: str = "controller_restart",
    ) -> bool:
        """Stage one controller-owned restart for a later flowchart callback.

        ``hxe_calls_done`` has no documented microcode-error return contract.
        The same delayed flowchart authority is also used after a poisoned
        mutation generation. The owning decompile controller initiates a
        follow-up pass; flowchart consumes this request and returns ``MERR_REDO``.
        """
        generation = int(self.evidence_generation)
        ...
        self.pending_generated_restart_family = str(evidence_family)
        self._observe_transition(
            operation="generated_restart_requested",
            previous_generation=generation,
            evidence_family=str(evidence_family),
