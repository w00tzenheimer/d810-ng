...
def overlaps_state_operand(
    ...
    *,
    ...
    if destination_size is None or destination_size <= 0:
        return True
    return (
        destination.key < operand.key + 4
        and operand.key < destination.key + destination_size
    )


@dataclass(frozen=True, slots=True)
class StorageDefinition:
    """Last definition, distinguishing absence from an unknown value.

    An unknown write or ABI clobber terminates a reaching-constant walk.
    Only an absent definition allows the walk to inspect a predecessor.
    """
