...
class FragmentProjectionFailure(ValueError):
    """Typed failure to derive projection evidence for one obligation."""
...
class FragmentProjectionBlockInput:
    """Read-only evidence for one already-existing or prepared physical block."""
...
class FragmentProjectionInput:
    """All immutable evidence required to project a fragment without live reads."""
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "data_flow_relations", tuple(self.data_flow_relations))
        ...
        object.__setattr__(
            self, "terminal_effect_diagnostics", tuple(self.terminal_effect_diagnostics)
        )
        object.__setattr__(
            self, "clone_source_instructions", tuple(self.clone_source_instructions)
        )
...
def project_fragment(
    ...
    if inventory.plan_id != plan.plan_id or inventory.atomic_group_id != plan.atomic_group_id:
    ...
    successors = {block_id: list(block.successors) for block_id, block in evidence.items()}
    kinds = {block_id: block.kind for block_id, block in evidence.items()}
    positions = {block_id: block.physical_position for block_id, block in evidence.items()}
    adjacency = {
        block_id: block.adjacent_fallthrough_target_id
        for block_id, block in evidence.items()
    }
    instruction_eas = {block_id: block.instruction_eas for block_id, block in evidence.items()}
    flag_write_eas = {block_id: block.flag_write_eas for block_id, block in evidence.items()}
    ...
    for block in plan.blocks:
        ...
        if block.materialization is FragmentBlockMaterialization.IMPORT_NATIVE and prepared:
    ...
    for operation in plan.operations:
        ...
        fallthrough_edges = tuple(
            edge for edge in operation.edges
            if edge.role in {SemanticEdgeRole.CALL_FALLTHROUGH, SemanticEdgeRole.CONDITIONAL_FALLTHROUGH}
        )
        ...
        for edge in operation.edges:
            ...
            if helper_id not in successors:
                ...
                fallthrough_helpers.append(ProjectedFallthroughHelper(
    ...
    for item in inventory.items:
        if item.predecessor_block_id not in successors or item.original_block_id not in successors:
            raise FragmentProjectionFailure(
                FragmentValidationPostcondition.ROOT_AUTHORITY,
                item.root_block_id,
        ...
        if item.requires_helper:
            helper_id = f"root-fallthrough-helper:{item.predecessor_block_id}:{item.root_block_id}"
            positions[helper_id] = positions[item.predecessor_block_id] + 1
            for block_id in tuple(positions):
                if block_id != helper_id and positions[block_id] >= positions[helper_id]:
                    positions[block_id] += 1
            ...
            root_helpers.append(ProjectedRootFallthroughHelper(
                helper_block_id=helper_id,
    ...
    projected_blocks = tuple(
        ...
        for block_id in sorted(successors, key=lambda value: (positions[value], value))
    )
...
def fragment_cfg_projection(
    ...
    """Normalize a serial-free fragment projection for the portable CFG contract."""
    ordered = tuple(sorted(projection.blocks, key=lambda block: block.physical_position))
    serial_by_id = {block.block_id: index for index, block in enumerate(ordered)}
    blocks = {
        serial_by_id[block.block_id]: BlockSnapshot(
            serial=serial_by_id[block.block_id],
            block_type=-1,
            succs=tuple(serial_by_id[target] for target in block.successors if target in serial_by_id),
            preds=tuple(serial_by_id[source] for source in block.predecessors if source in serial_by_id),
            flags=0,
            start_ea=0,
            insn_snapshots=(),
            kind=block.kind,
    ...
    return CfgProjection(
        ...
        graph=FlowGraph(
            blocks=blocks,
            entry_serial=serial_by_id[projection.entry_block_id],
            func_ea=0,
            metadata={"fragment_block_ids": tuple(block.block_id for block in ordered)},
        ),
        focus_refs=tuple(PlanBlockRef(plan.plan_id, block.block_id) for block in ordered),
    )
