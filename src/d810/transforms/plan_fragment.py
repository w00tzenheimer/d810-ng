"""Shared rewrite-plan data types for the unflattening engine.

These types (``PlanFragmentMetadata``, ``OwnershipScope``, ``BenefitMetrics``,
``PlanFragment``, ``StageResult``, ``VerificationGate``) are the canonical,
layer-low home for the plan-fragment vocabulary consumed across the passes,
families, and backends layers.

All types in this module are pure Python, so they can be exercised by unit
tests without an IDA environment.  The module lives in ``d810.transforms``
