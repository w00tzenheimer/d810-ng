...
This module deliberately stops at the portable transaction boundary: live CFG
objects and backend-specific bindings belong behind a participant implementation.
"""
...
class TransactionAttemptId:
    """One realization attempt for an immutable plan."""
    ...
    @classmethod
    def new(cls, plan_id: str) -> TransactionAttemptId:
        return cls(plan_id=plan_id, attempt_id=uuid4().hex)
...
@dataclass(frozen=True, slots=True)
class CfgProjection:
    """A typed, immutable plan projection over a portable CFG snapshot."""
...
class CfgTransactionPhase(str, Enum):
    """The portable transaction lifecycle, including terminal failure states."""
...
class CfgTransactionFailure:
    """Terminal failure evidence; this record is deliberately not an exception."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.first_failed_obligation is not None:
            _require_identifier(self.first_failed_obligation, "first_failed_obligation")
...
class CfgGenerationPoisoned(RuntimeError):
    """Unwind a callback after an SDK write made its MBA generation unsafe."""
...
class CfgTransactionParticipant(Protocol):
    ...
    def bind(
        self, prepared: PreparedCfgTransaction, identity_index: object
    ) -> BoundCfgTransaction: ...
