...
class RhadAbsoluteConstantMaterialization:
    ...
    def __post_init__(self) -> None:
        ...
        encoding_contract = {
            ...
            RhadAbsoluteConstantEncoding.MOVZX_R32_BYTE_ABSOLUTE: (
                RhadOperationVariant.MOVZX_ABSOLUTE,
                7,
                7,
                8,
        ...
        if (
            self.operation_variant is RhadOperationVariant.MOVZX_ABSOLUTE
            and publication_envelope
            is not RhadConstantPublicationEnvelope.GENERATED_BYTE_LOAD_ZERO_EXTEND
        ):
            raise RhadCompilerRejection(
                "Rhad movzx-absolute requires its generated byte-load zero-extend "
                "envelope"
            )
        ...
        if reference_raw_value != encoded_value:
            raise RhadCompilerRejection(
        ...
        if materialized_value != expected_materialized_value:
            if self.operation_variant is RhadOperationVariant.MOVZX_ABSOLUTE:
                raise RhadCompilerRejection(
                    "Rhad movzx-absolute zero-extended byte value differs from "
                    "reference data"
                )
            raise RhadCompilerRejection(
                "Rhad absolute constant data bytes and materialized value differ"
            )
        if not 0 <= materialized_value < (1 << source_width_bits):
            raise RhadCompilerRejection(
                "Rhad absolute materialized value exceeds its source width"
            )
        dependencies = tuple(
            _identifier(value, "Rhad dependency") for value in self.depends_on
        )
