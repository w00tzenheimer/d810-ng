...
class FragmentConstantPublicationEnvelope(str, Enum):
    ...
    GENERATED_BYTE_LOAD_ZERO_EXTEND = "generated_byte_load_zero_extend"
...
class FragmentAbsoluteConstantMaterialization:
    ...
    def __post_init__(self) -> None:
        ...
        source_width_bits = int(self.source_width_bits)
        destination_width_bits = int(self.destination_width_bits)
        ...
        (
            ...
            expected_source_width_bits,
            expected_destination_width_bits,
            expected_reference_read_width_bits,
        ) = encoding_contract
        ...
        if (
            ...
            raise FragmentPlanRejected(
                "absolute materialization widths differ from its typed encoding"
            )
        ...
        if (
            ...
            and publication_envelope
            is not FragmentConstantPublicationEnvelope.GENERATED_BYTE_LOAD_ZERO_EXTEND
        ):
            raise FragmentPlanRejected(
                "movzx-absolute materialization requires its generated byte-load "
                "zero-extend envelope"
            )
        encodings = (
        ...
        reference_raw_value = int.from_bytes(
        ...
        if not 0 <= constant_value < (1 << source_width_bits):
            raise FragmentPlanRejected(
                "absolute materialized value exceeds its source width"
            )
        ...
        object.__setattr__(self, "source_width_bits", source_width_bits)
        object.__setattr__(self, "destination_width_bits", destination_width_bits)
        object.__setattr__(
            self,
            "reference_read_width_bits",
            reference_read_width_bits,
