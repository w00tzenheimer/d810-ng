"""Migration shim: ``d810.cfg.mod_claims`` -> ``d810.transforms.mod_claims`` (dissolution, llr-lyly).

sys.modules alias preserving the old import path; re-exports public AND
private symbols.  Deleted in Phase Z once consumers repoint.

This is a *transforms*-layer module (planner/backend execution surface),
so the canonical home is :mod:`d810.transforms.mod_claims`.  ``d810.cfg`` sits BELOW
...
repoint to ``d810.transforms.mod_claims`` directly in Phase T-wave-repoint.
"""
import importlib
import sys

_canonical = importlib.import_module("d810.transforms.mod_claims")
