"""Cleanup strategy for local guarded constant-state machines."""
...
class GuardedStateMachineStrategy:
    """Engine strategy for local guarded constant-state machine shells."""

    name = "guarded_state_machine"
    family = FAMILY_CLEANUP

    def is_applicable(self, snapshot: AnalysisSnapshot) -> bool:
        return bool(extract_guarded_state_machine_fixes(snapshot.flow_graph))

    def plan(self, snapshot: AnalysisSnapshot) -> PlanFragment | None:
        fixes = extract_guarded_state_machine_fixes(snapshot.flow_graph)
        if not fixes:
            return None
        modifications = build_guarded_state_machine_modifications(fixes)
