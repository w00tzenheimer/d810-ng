...
class HodurUnflattener(ComposedUnflatteningRule):
            ...
            for mod in frag.modifications:
                ...
                if ppt is not None:
                    for pred, target in ppt:
                        self._reconstruction_live_blocks.add(int(pred))
                        self._reconstruction_live_blocks.add(int(target))
        ...
        *,
        ...
        try:
            ...
            while diag_queue:
                ...
                if serial in diag_visited or serial < 0 or serial >= self.mba.qty:
                    continue
            ...
            gutted_serials = all_serials - diag_visited - {self.mba.qty - 1}
            claimed: set[int] = set()
        ...
        *,
        ...
        if dead_cleanup_applied == 0:
            ...
            for serial in bst_serials:
                block = self.mba.get_mblock(serial)
                if block is not None:
                    self._last_bst_block_eas.add(block.start)
    ...
    def _run_terminal_byte_mbl_keep_hook(self, snapshot: AnalysisSnapshot) -> None:
        ...
        try:
            ...
            if tagged:
                self._capture_intermediate_snapshot("post_mbl_keep_terminal_byte")
        except Exception:
            unflat_logger.debug(
    ...
    def _run_reachable_mbl_keep_hook(self) -> None:
        ...
        if not (self.MBL_KEEP_ENABLED and self.mba is not None and self.mba.qty > 1):
            return
