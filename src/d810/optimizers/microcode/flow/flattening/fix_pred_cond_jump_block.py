...
class FixPredecessorOfConditionalJumpBlock(GenericUnflatteningRule):
        ...
        if jmp_ins.opcode == m_jnz:
            ...
            is_jmp_always_taken = all(
                [
                    ...
                    > unsigned_to_signed(compared_value, compared_value_size)
                    for possible_value in pred_comparison_values
                ]
            )
            is_jmp_never_taken = all(
    ...
    def sort_predecessors(self, blk):
        ...
        for pred_serial in blk_preset_list:
            ...
            unflat_logger.info(
                "Pred {0} has {1} possible path ({2} different cst): {3}".format(
                    pred_blk.serial,
                    len(pred_values),
                    len(set(pred_values)),
                    pred_values,
            ...
            if is_jmp_always_taken and is_jmp_never_taken:
                ...
                unflat_logger.error(
                    "It seems that I am stupid: '{0}' is always taken and not taken when coming from {1}: {2}".format(
                        format_minsn_t(blk.tail), pred_blk.serial, pred_values
                    )
                )
                pred_jmp_unk.append(pred_blk)
                continue
            if is_jmp_always_taken:
                unflat_logger.info(
                    "It seems that '{0}' is always taken when coming from {1}: {2}".format(
                        format_minsn_t(blk.tail), pred_blk.serial, pred_values
                    )
                )
            ...
            if is_jmp_never_taken:
                unflat_logger.info(
                    "It seems that '{0}' is never taken when coming from {1}: {2}".format(
                        format_minsn_t(blk.tail), pred_blk.serial, pred_values
                    )
                )
    ...
    def analyze_blk(self, blk: mblock_t) -> int:
        ...
        unflat_logger.info(
            "Block {0} has {1} preds: {2} always jmp, {3} never jmp, {4} unk".format(
                blk.serial,
                blk.npred(),
        ...
        if len(pred_jmp_always_taken) > 0:
            if self.dump_intermediate_microcode:
                dump_microcode_for_debug(
                    self.mba,
                    self.log_dir,
                    "{0}_{1}_before_jmp_always_fix".format(
                        self.cur_maturity_pass, blk.serial
                    ),
                )
            ...
            if self.dump_intermediate_microcode:
                dump_microcode_for_debug(
                    self.mba,
                    self.log_dir,
                    "{0}_{1}_after_jmp_always_fix".format(
                        self.cur_maturity_pass, blk.serial
                    ),
                )
        ...
        if len(pred_jmp_never_taken) > 0:
            if self.dump_intermediate_microcode:
                dump_microcode_for_debug(
                    self.mba,
                    self.log_dir,
                    "{0}_{1}_before_jmp_never_fix".format(
                        self.cur_maturity_pass, blk.serial
                    ),
                )
            ...
            if self.dump_intermediate_microcode:
                dump_microcode_for_debug(
                    self.mba,
                    self.log_dir,
                    "{0}_{1}_after_jmp_never_fix".format(
                        self.cur_maturity_pass, blk.serial
                    ),
                )
    ...
    def check_if_rule_should_be_used(self, blk: mblock_t) -> bool:
        ...
        if self.cur_maturity not in self.maturities:
            return False
        if (self.DEFAULT_MAX_PASSES is not None) and (
            self.cur_maturity_pass >= self.DEFAULT_MAX_PASSES
        ):
            return False
        if (blk.tail is None) or blk.tail.opcode not in JMP_OPCODE_HANDLED:
            return False
