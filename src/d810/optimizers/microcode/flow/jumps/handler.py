...
def _discarded_corridor_has_side_effect_payload(
    ...
    *,
    ...
    "Return whether the branch being removed owns local payload side effects.\n\n    JumpFixer normally treats opaque branches proven by a jump-folding rule as\n    disposable.  That is fine for pure dispatcher-routing blocks, but OLLVM\n    state-map lowering can leave branch-local payload corridors behind: one arm may\n    contain a memory store that preanalysis has identified as semantic work.  If a\n    later opaque fold discards that arm, IDA DCE erases the payload and the final\n    pseudocode looks cleaner while no longer representing both semantic outcomes.\n    When ``required_constant_markers`` is supplied, this guard preserves only stores\n    whose value expression carries one of those semantic constants; generic\n    stack-zeroing and local bookkeeping stores remain foldable.\n\n    Keep this bounded and local.  The guard looks only through a small corridor\n    rooted at the discarded successor and stops when it reaches the successor\n    chosen by the fold.  It is enabled by profile config rather than globally.\n"
...
class JumpFixer(FlowOptimizationRule):
    ...
    def configure(self, kwargs):
        ...
        if "enabled_rules" in self.config.keys():
            rules_by_name = {rule.name: rule for rule in self.known_rules}
            enabled_rule_names = tuple(self.config["enabled_rules"])
            for rule_name in enabled_rule_names:
                rule = rules_by_name.get(rule_name)
                if rule is None:
                    optimizer_logger.warning(
                        "JumpFixer configured unknown rule %s", rule_name
                    )
                    continue
                rule.configure()
                self.rules.append(rule)
                optimizer_logger.debug("JumpFixer enables rule %s", rule.name)
            for rule in self.known_rules:
                if rule.name not in enabled_rule_names:
                    optimizer_logger.debug("JumpFixer disables rule %s", rule.name)

    def _jump_fold_discards_side_effect_payload(
        self,
        blk: ida_hexrays.mblock_t,
        instruction: ida_hexrays.minsn_t,
            ...
            try:
                ...
                if new_ins:
                    if (
                        ...
                        continue
                    if self._jump_fold_discards_side_effect_payload(
                        blk,
                        blk.tail,
                        new_ins,
                    ):
