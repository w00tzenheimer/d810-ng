...
class MopHistory(object):
    ...
    def get_mop_constant_value(
        ...
        # ``eval_mop`` is EXACT-or-``None``: a value descending from a call the
        # emulator MODELED rather than computed comes back unresolved on its own
        # (ticket d81-1t9x), so no per-consumer taint check is needed here.  An
        # unrelated (dead) synthetic result still no longer discards the whole
        # path (ticket d81-0xzp).
