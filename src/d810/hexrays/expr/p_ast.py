...
class AstNode(AstBase):
    def __init__(
        ...
        self._depth_sig_cache: dict[
            int, list[str]
        ] = {}  # Cache for get_depth_signature
...
class AstConstant(AstLeaf):
    ...
    def _copy_mops_from_ast(self, other, read_only: bool = False):
        ...
        if not read_only:
            if other_mop is None:
                self.mop = None
            elif isinstance(other_mop, MopSnapshot):
                self.mop = other.mop
            else:
                self.mop = MopSnapshot.from_mop(other_mop)
