...
class NativePatchPlanRequest:
    ...
    The direct Tigress materializer does not execute this proposal. Keeping
    the type lets the manager continue to configure generic native-patch
    infrastructure without making this profile depend on its incomplete xref
    state model.
    """
...
def _materialize_indirect_label_targets_legacy(
    *,
    function_ea: int,
    table_address: int,
    table_count: int,
...
def execute_legacy_indirect_materialization(
    request: NativePatchPlanRequest,
) -> IndirectLabelMaterializationResult:
    """Manager-selected compatibility executor for the proven Tigress route."""
    return _materialize_indirect_label_targets_legacy(
