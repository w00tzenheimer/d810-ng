...
class MbaCfgTransactionAuthorityObserved:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.attempt_id, TransactionAttemptId):
            raise TypeError("CFG transaction observation requires typed attempt authority")
        ...
        if any(
            item.plan_ref.plan_id != self.attempt_id.plan_id
            for item in self.plan_bindings
        ):
            raise ValueError("CFG transaction plan binding authority differs")
        if any(item.attempt_id != self.attempt_id for item in self.reservations):
            raise ValueError("CFG transaction reservation authority differs")
        if any(item.attempt_id != self.attempt_id for item in self.creation_receipts):
            raise ValueError("CFG transaction creation authority differs")
        declared_refs = set(self.plan_refs)
        ...
        if any(item.plan_ref not in declared_refs for item in self.reservations):
            raise ValueError("CFG transaction reservation is not declared")
        if any(item.plan_ref not in declared_refs for item in self.creation_receipts):
            raise ValueError("CFG transaction creation is not declared")
        ...
        if any(
            plan_ref.plan_id != self.attempt_id.plan_id
            or int(before) < 0
            or int(after) != int(before) + 1
            for plan_ref, before, after in self.creation_quantities
        ):
            raise ValueError("CFG transaction creation quantity authority differs")
...
class MbaMutationGateway:
    ...
    def __post_init__(self) -> None:
        ...
        if self.identity_index.maturity is None:
            self.identity_index.maturity = int(self.maturity)
            unknown_suffix = f":munknown:g{self.identity_index.generation}"
            if str(self.identity_index.snapshot_id).endswith(unknown_suffix):
                self.identity_index.snapshot_id = (
                    f"{self.session_id}:m{self.maturity}:g{self.identity_index.generation}"
                )
        elif int(self.identity_index.maturity) != int(self.maturity):
            raise ValueError("mutation gateway and identity index maturities differ")
    ...
    def plan_creation_receipts(self) -> tuple[PlanBlockCreationReceipt, ...]:
        """Return exact plan-to-live creation proofs for the current attempt."""
    ...
    def _record_clean_fragment_failure(
        ...
        *,
        ...
        self._emit_cfg_transaction_phase(CfgTransactionPhase.REJECTED_CLEAN, failure=failure)
        return failure
    ...
    def _poison_fragment_generation(
        ...
        *,
        ...
        # This invalidates transaction-local logical versions only. It never
        # touches the already-mutated MBA or attempts physical recovery.
        self._transaction_failure = failure
    ...
    def begin_batch(
        ...
        *,
        ...
        if transaction_attempt is not None:
            expected_plan_id = (
                fragment_plan.plan_id
                if fragment_plan is not None
                else patch_plan_id
            )
            if expected_plan_id is None or transaction_attempt.plan_id != expected_plan_id:
                raise ValueError("transaction attempt does not match the active plan")
            if any(ref.plan_id != transaction_attempt.plan_id for ref in patch_plan_refs):
        ...
        if transaction_attempt is not None and self._current_transaction_attempt is None:
            self._current_transaction_attempt = transaction_attempt
            self._cfg_phase_index = 0
            self._cfg_plan_refs = patch_plan_refs
            if not self._cfg_plan_refs and fragment_plan is not None:
                self._cfg_plan_refs = tuple(
                    PlanBlockRef(fragment_plan.plan_id, block.block_id)
                    for block in fragment_plan.blocks
                )
        batch_id = (
            uuid.uuid4().hex
    ...
    def _begin_semantic_fragment_batch(
        ...
        if prepared_fragment is not None and (
            prepared_fragment.authority.attempt_id is not transaction_attempt
        ...
        self.begin_batch(
            ...
            patch_plan_id=(None if patch_plan is None else patch_plan.plan_id),
            patch_plan_refs=(
                ()
                if patch_plan is None
                else self._cfg_plan_refs
            ),
        )
    ...
    def reserve_plan_block(
        ...
        *,
        ...
        self._require_active()
        if attempt_id.attempt_id != str(self._active_batch_id):
    ...
    def bind_reserved_plan_block(
        ...
        *,
        ...
        """Bind a planned insert and synchronously shift transaction coordinates."""
        self._require_active()
        if attempt_id.attempt_id != str(self._active_batch_id):
        ...
        quantity_after = self.identity_index.transaction_quantity(
            str(self._active_batch_id)
        )
        self._cfg_creation_quantities[plan_ref] = (
            quantity_after - 1,
            quantity_after,
        )
    ...
    def record_insert(
        ...
        *,
        ...
        self._require_active()
        if created is None:
            raise ValueError("planned insertion requires a reserved plan owner")
    ...
    def record_observed_insert(
        ...
        *,
        ...
        """Record an anonymously discovered SDK block without claiming ownership."""
        self._require_active()
        observed = self.identity_index.create_observed_ephemeral_handle()
        self.identity_index.record_insert(
            transaction_id=str(self._active_batch_id),
            insertion_serial=int(insertion_serial),
            created=observed,
...
__all__ = [
    "MbaCfgPlanBlockBindingObserved",
