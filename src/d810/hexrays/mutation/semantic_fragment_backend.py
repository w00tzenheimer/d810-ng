...
class SemanticFragmentBackendState:
    ...
    return_carrier_constructions: dict[
        str, PreparedReturnCarrierConstruction
...
def _prepare_native_bodies(
    modifier: DeferredGraphModifier,
    plan: FragmentPlan,
) -> tuple[PreparedNativeBodyPreparation, ...]:
    """Prepare every native body before the first destination-MBA mutation."""
    materializer = _native_body_materializer(modifier, plan)
    if materializer is None:
        return ()
    result = tuple(
        materializer.prepare_native_body(plan=plan, native_body=native_body)
        for native_body in plan.native_bodies
    )
    if any(not isinstance(item, PreparedNativeBodyPreparation) for item in result):
        raise SemanticFragmentBackendRejected(
...
def _stage_native_bodies(
    ...
    *,
    reference_version: LogicalBlockVersion,
    preparations: tuple[PreparedNativeBodyPreparation, ...],
    native_body_id: str,
) -> None:
    if not plan.native_bodies:
        if preparations:
    ...
    preparation_by_body_id = {item.fact.body_id: item for item in preparations}
    if (
...
def _create_empty_block(
    ...
    *,
    ...
    staged = modifier._stage_empty_semantic_block(
        reference_version=reference_version,
        plan_ref=PlanBlockRef(state.plan_id, block.block_id),
    )
    gateway = _gateway(modifier)
...
def _realize_operations(
    ...
    items = plan.operations if operation_steps is None else tuple(operation_steps)
    for item in items:
        operation = item.operation if isinstance(item, PatchFragmentOperation) else item
        helper_plan_ref = (
            item.fallthrough_helper_ref
            if isinstance(item, PatchFragmentOperation)
            else None
        )
...
def _prepared_operand_shape(
    ...
    """Immutable primitive descriptor for a prepared SDK operand payload."""
    storage = source.storage_identity
    binding_kind = None if storage is None else storage.kind.value
    binding_offset = None if storage is None else int(storage.offset)
    constant = None if source.constant is None else int(source.constant)
    return (
        source.kind.value,
        int(source.width),
        binding_kind,
...
_TERMINAL_RETURN_CLOSING_OPCODES = frozenset(
    {
...
def _terminal_return_can_append_after_opcode(opcode: int) -> bool:
    """Keep immutable preflight and live realization on one tail predicate."""
    opcode = int(opcode)
    return not ida_hexrays.is_mcode_jcond(opcode) and (
        opcode not in _TERMINAL_RETURN_CLOSING_OPCODES
    )
...
def _materialize_terminal_return(
    ...
    if matches:
        ...
        tail = block.tail
        if tail is not None and not _terminal_return_can_append_after_opcode(
            int(tail.opcode)
        ):
            raise SemanticFragmentBackendRejected(
    ...
    *,
    ...
    for expected_binding in state.preflight_projection.identity_bindings:
        ...
        if block_id in projection_bindings:
            continue
        bound = gateway.identity_index.resolve_logical_ref(
            LogicalBlockRef(
                session_id=gateway.session_id,
                proxy_token=expected_binding.logical_owner_id,
                version=expected_binding.version,
            ),
            transaction_id=transaction_id,
        )
        block = None if bound is None else modifier.mba.get_mblock(int(bound.serial))
        proxy = (
            None
            if bound is None
            else gateway.identity_index.logical_proxy_for_handle(bound.handle)
        )
        version = None if proxy is None else proxy.resolve(transaction_id=transaction_id)
        if block is None or proxy is None or version is None or bound is None:
            raise SemanticFragmentBackendRejected(
    ...
    data_flow_relations = _project_data_flow_relations(
        modifier, plan, live_by_id, ids_by_serial
    )
    value_ranges = _project_value_ranges(state, plan, live_by_id)
    return_carriers = _project_return_carriers(modifier, plan, state, live_by_id)
    terminal_effect_diagnostics = _project_terminal_effect_diagnostics(
        modifier, plan, state, live_by_id
    )
    terminal_returns = _project_terminal_returns(
        plan, state, live_by_id, return_carriers
    )
...
def plan_semantic_fragment_root_inventory(
    ...
    for root_block_id in plan.roots:
        ...
        for predecessor_serial in predecessor_serials:
            ...
            root_subject = (
                f"{root_block_id}<-{predecessor_block_id}"
                f"@0x{int(predecessor.start):X}"
            )
            try:
...
def snapshot_semantic_fragment_inputs(
    ...
    """Capture immutable preflight evidence before the gateway opens a batch."""
    gateway = modifier._mutation_gateway
    if gateway is None:
        raise SemanticFragmentBackendRejected(
    ...
    for planned in plan.blocks:
        ...
        if serial in ids_by_serial:
            ...
            raise FragmentProjectionFailure(
                FragmentValidationPostcondition.IDENTITY_OWNERSHIP,
                planned.block_id,
                f"snapshot blocks {ids_by_serial[serial]!r} and {planned.block_id!r} "
                f"alias blk{serial}@0x{anchor:X}",
            )
        proxy = gateway.identity_index.logical_proxy_for_handle(rebound.block.handle)
    ...
    for serial in range(int(modifier.mba.qty)):
        ...
        proxy = None if handle is None else gateway.identity_index.logical_proxy_for_handle(handle)
        version = None if proxy is None else proxy.resolve()
    ...
    if 0 not in ids_by_serial:
        handle = gateway.identity_index.handle_for_serial(0)
        live = modifier.mba.get_mblock(0)
        proxy = None if handle is None else gateway.identity_index.logical_proxy_for_handle(handle)
        version = None if proxy is None else proxy.resolve()
        if live is None or proxy is None or version is None:
            raise FragmentProjectionFailure(
        ...
        entry_id = f"function-entry:{proxy.proxy_token}"
        live_by_id[entry_id] = live
        ids_by_serial[0] = entry_id
        binding_by_id[entry_id] = ProjectedIdentityBinding(
            block_id=entry_id,
            logical_owner_id=proxy.proxy_token,
            version=version.version_id.version,
            generation=version.generation,
    ...
    flag_writes: dict[str, frozenset[int]] = {}
            ...
            FragmentProjectionBlockInput(
                ...
                successors=(
                    ...
                    if kind is BlockKind.ZERO_WAY
                        ...
                        for serial in live.succset
                    )
                ),
                predecessors=tuple(
                    ids_by_serial.get(int(serial), _unowned_endpoint(modifier, int(serial)))
                    for serial in live.predset
                ),
    ...
    next_position = max((block.physical_position for block in blocks), default=-1) + 1
    for planned in plan.blocks:
        ...
        prepared_rows = () if preparation is None else preparation.payload.rows
        matching_rows = tuple(
            row for row in prepared_rows if len(row) == 3 and str(row[0]) == planned.block_id
        )
        instruction_rows = () if len(matching_rows) != 1 else tuple(matching_rows[0][2])
        ...
        imported_flag_writes: set[int] = set()
        for native_ea, instruction in instruction_rows:
            try:
                if instruction_writes_condition_codes(instruction):
                    imported_flag_writes.add(int(native_ea))
            except ConditionCodeQueryUnavailable as exc:
                raise FragmentProjectionFailure(
                    FragmentValidationPostcondition.FLAG_CORRIDOR_INTEGRITY,
                    plan.flag_corridors[0].corridor_id,
                    f"prepared native block {planned.block_id!r} has unclassifiable flags",
                ) from exc
        blocks.append(
            FragmentProjectionBlockInput(
                block_id=planned.block_id,
                ...
                instruction_eas=exact_eas,
                flag_write_eas=frozenset(imported_flag_writes),
            )
        )
        next_position += 1
    ...
    site_eas_by_block = {
        block.block_id: frozenset(block.instruction_eas) for block in blocks
    }
    for planned in plan.blocks:
        if planned.role is FragmentBlockRole.REPLACEMENT:
            site_eas_by_block[planned.block_id] = site_eas_by_block.get(
                str(planned.replaces_block_id), frozenset()
            )
    ...
    for carrier in plan.return_carriers:
        ...
        state_matches = tuple(
            index for index, (ea, _insn) in enumerate(rows)
            if int(ea) == carrier.state_write_ea
        )
        carrier_matches = tuple(
            (index, insn) for index, (ea, insn) in enumerate(rows)
        ...
        if can_insert or existing_matches:
            ...
            terminal_diagnostics.append(
                ProjectedTerminalEffectDiagnostic(
                    ...
                    reason="prepared carrier anchors or operation do not fit atomically",
                )
            )
    ...
    for terminal in plan.terminal_returns:
        ...
        can_insert = bool(
            ...
            and (
                not rows
                or _terminal_return_can_append_after_opcode(
                    int(rows[-1][1].opcode)
                )
            )
        )
        ...
        if can_insert or rewritable_tail:
    ...
    projection_input = FragmentProjectionInput(
        snapshot_id=(
            f"semantic-fragment:{gateway.session_id}:{gateway.generation}:{plan.plan_id}"
        ),
...
def _reserve_root_fallthrough_helpers(
    modifier: DeferredGraphModifier,
    state: SemanticFragmentBackendState,
    root_steps: tuple[PatchFragmentRootPublication, ...],
) -> None:
    gateway = _gateway(modifier)
    for step in root_steps:
        ...
        if step.fallthrough_helper_ref is None:
            raise SemanticFragmentBackendRejected(
                "root helper PatchStep lacks PlanBlockRef authority"
            )
        if step.predecessor_ref is None:
            raise SemanticFragmentBackendRejected(
                "root helper PatchStep lacks predecessor authority"
            )
...
def observe_published_semantic_fragment(
    ...
    """Compatibility semantic view of the authoritative graph observation."""
    return observe_published_semantic_fragment_graph(modifier, plan).semantics
...
def realize_semantic_patch_plan(
    ...
    for carrier_id, construction in construction_by_id.items():
        ...
        if _prepared_operand_shape(operand, construction.source) != construction.operand_shape:
            raise SemanticFragmentBackendRejected(
