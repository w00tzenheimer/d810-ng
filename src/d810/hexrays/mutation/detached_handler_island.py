...
class _ComputedBranchNormalizationPlan:
    """Backend realization of one proof-owned computed branch."""
...
def _relocate_imported_owner(mba: object, owner: object) -> object | None:
    ...
    for imported_ea in owner.owned_instruction_eas:
        ...
        if len(relocated) == 1:
            surviving_origins.append((int(native_ea), int(imported_ea), relocated[0]))
...
def _blocks_containing_native_ea(mba: object, native_ea: int) -> tuple[object, ...]:
    ...
    matches = {
        int(block.serial): block for block in _blocks_containing_ea(mba, int(native_ea))
    }
    ...
    for (
        ...
        if int(owner_identity) != int(mba_identity) or not int(start_ea) <= int(
            native_ea
        ) < int(end_ea):
            continue
...
def find_unique_live_block_by_native_ea(
    ...
    if not competing_blocks or not all(
        not _instructions(block) and int(block.type) == int(ida_hexrays.BLT_XTRN)
...
def find_materialized_handler_block_by_native_ea(
    ...
    *,
    ...
    competitors = tuple(
        block for block in matches if int(block.serial) != int(imported_owner.serial)
    )
    if not all(
        _block_exactly_owns_native_ea(mba, block, target_ea) for block in competitors
    ):
        logger.info(
            ...
            int(_unique_block_native_ea(imported_owner) or int(imported_owner.start)),
            tuple(
                "blk%d@0x%X"
                % (
                    int(block.serial),
...
def _generated_superseded_transfer_carrier_block_ids(
    *,
    plan: FragmentPlan,
    native_body: FragmentNativeBody,
) -> set[str]:
    """Return the exact plan-owned imported carriers replaced at publication."""
    return (
...
class PreoptUnionSemanticNativeBodyMaterializer:
    ...
    def _compose_generated_reference_templates(
        ...
        owned_serial_by_entry: dict[int, int] = {}
        next_serial = 0
        for template, owned_blocks in zip(components, owned_blocks_by_component):
            ...
            for block in owned_blocks:
                ...
                previous_entry_serial = owned_serial_by_entry.setdefault(
                    int(block.native_entry_ea), next_serial
                )
                if int(previous_entry_serial) != next_serial:
                    raise SemanticFragmentBackendRejected(
                        "GENERATED reference templates contain duplicate typed "
                        f"owned entry 0x{int(block.native_entry_ea):X}"
                    )
                next_serial += 1
        ...
        for component_index, (template, owned_blocks) in enumerate(
            ...
            for block in owned_blocks:
                ...
                unresolved_multi_exit_carrier = bool(
                    tail is not None
                    and int(tail.opcode) == int(ida_hexrays.m_ret)
                    and int(tail.ea) == int(block.native_entry_ea)
                    and int(block.block_type) == int(ida_hexrays.BLT_STOP)
                    and bool(int(block.block_flags) & int(ida_hexrays.MBL_FAKE))
                    and len(
                        tuple(
                            int(ea)
                            for ea in block.external_successor_eas
                            if int(ea) > 0
                        )
                    )
                    > 1
                ...
                for external_ea in block.external_successor_eas[
                    ...
                    external_ea = int(external_ea)
                    if external_ea <= 0:
                        continue
                    imported_successor = owned_serial_by_entry.get(external_ea)
                    if (
                        imported_successor is not None
                        and not unresolved_multi_exit_carrier
                    ):
                        internal_successors.append(int(imported_successor))
                    else:
                        external_successors.append(external_ea)
                rebased_blocks.append(
                    replace(
                        block,
                        source_serial=rebased_serials[int(block.source_serial)],
                        instructions=instructions,
                        successor_serials=tuple(dict.fromkeys(internal_successors)),
                        external_successor_eas=tuple(
                            dict.fromkeys(external_successors)
                        ),
                    )
                )
    ...
    def _preflight_stack_rebase(
        ...
        *,
        ...
        superseded_transfer_carrier_block_ids = (
            _generated_superseded_transfer_carrier_block_ids(
                plan=plan,
                native_body=native_body,
            )
        )
        ...
        for block_id, template_block in matched.items():
            ...
            if allow_unplanned_transfers:
                ...
                terminal_return_is_bound = bool(
                    self._generated_terminal_return_is_bound(
                        block_id=block_id,
                        template_block=template_block,
                        has_terminal_return=has_terminal_return,
                        superseded_transfer_carrier_block_ids=(
                            superseded_transfer_carrier_block_ids
                        ),
    ...
    @staticmethod
    def _generated_terminal_return_is_bound(
        *,
        block_id: str,
        template_block: DetachedSnippetBlockTemplate,
        has_terminal_return: bool,
        superseded_transfer_carrier_block_ids: set[str],
        preserved_native_transfer_block_ids: set[str],
    ) -> bool:
        """Accept exact external or internally stitched GENERATED carriers."""
        external_targets = tuple(
        ...
        return bool(
            ...
            or (
                ...
                and bool(external_targets or template_block.successor_serials)
            )
        )
    ...
    def _preflight_computed_branch_normalization(
        ...
        """Validate one proof-owned unresolved suffix and choose its branch sense."""
        label = (
            f"operation={operation.operation_id!r} "
            f"source=0x{int(template_block.native_entry_ea):X} "
            f"producer=0x{int(normalization.condition_producer_ea):X} "
    ...
    def _generated_preserved_transfer_authority(
        *,
        matched: Mapping[str, DetachedSnippetBlockTemplate],
        native_body: FragmentNativeBody,
        superseded_transfer_carrier_block_ids: Collection[str],
    ) -> tuple[
        ...
        for block_id in native_body.preserved_native_transfer_block_ids:
            ...
            unresolved_transfer_carrier = bool(
                ...
                and bool(int(template_block.block_flags) & int(ida_hexrays.MBL_FAKE))
            )
            if unresolved_transfer_carrier and len(internal_targets) <= 1:
                successors = tuple(
    ...
    def stage_generated_native_body(
        ...
        *,
        ...
        for block_id in native_body.block_ids:
            ...
            if (
                external_targets
                and block_id not in superseded_transfer_carrier_block_ids
            ):
...
def _backfill_owned_native_entries(
    ...
    *,
    ...
    for entry_ea in entries:
        if any(
            int(block.native_entry_ea) <= entry_ea < int(block.native_end_ea)
            for block in result
        ):
            continue
        ...
        if any(
            entry_ea < other_entry_ea < candidate_entry_ea for other_entry_ea in entries
        ):
            continue
...
def _capture_template_boundary_ports(
    ...
    for port in boundary_ports.conditional:
        ...
        old_taken_target_serial = (
            None
            if port.old_taken_target_ea is None or port.old_taken_target_owner is None
            else bound_serial(
...
def _capture_detached_snippet_template(
    ...
    for serial, block in sorted(included.items()):
        ...
        for instruction in instructions:
            ...
            authoritative_frame_offsets = authoritative_stack_frame_offsets_by_ea.get(
                int(instruction.ea),
                (),
            )
            authoritative_frame_identity = (
                int(authoritative_frame_offsets[0])
                if len(source_vd_offsets) == 1 and len(authoritative_frame_offsets) == 1
                else None
            )
    ...
    for block in blocks:
        ...
        for instruction in block.instructions:
            ...
            if (
                ...
                deltas_by_call.setdefault(int(instruction.ea), set()).add(-pushed_bytes)
            pushed_bytes = 0
    return tuple(
        sorted(
...
def detached_preopt_call_stack_points(
    ...
    deltas_by_call: dict[int, set[int]] = {}
    for (owner_ea, _target_ea), template in _PREOPT_UNION_SNIPPET_TEMPLATES.items():
        if int(owner_ea) != int(function_ea):
            continue
        for call_ea, stack_pointer_delta in template.direct_call_stack_pointer_deltas:
            deltas_by_call.setdefault(int(call_ea), set()).add(int(stack_pointer_delta))
    return tuple(
        sorted(
            (call_ea, next(iter(deltas)))
            for call_ea, deltas in deltas_by_call.items()
...
def _prepare_terminal_return_carrier_instruction(
    ...
    destination_vd_offset = int(mba.stkoff_ida2vd(int(source_stack_ida_offset)))
    ...
    if (
        ...
        for successor_serial in successor_serials:
            ...
            other_successors = tuple(
                serial for serial in successor_serials if serial != successor_serial
            )
            if (
    ...
    *,
    ...
    def bind_conditional_source(
        ...
        logical_source_complete = (
            ...
            and (
                port.predicate_stack_value is not None or port.condition_code in (4, 5)
            )
        )
        ...
        if logical_source_complete:
            ...
            logical_source_owner = port.logical_source_owner or port.source_owner
            source = bind(
        ...
        for record in template.boundary_ports.conditional:
            ...
            if (
                ...
                resolver_return_envelope = bool(
                    ...
                    and int(return_block.instructions[0].ea) == int(port.predicate_ea)
...
def _rebind_boundary_port_block(
    ...
    *,
    ...
    instruction_backed = tuple(block for block in matches if _instructions(block))
    ...
    for mutation in batch.direct:
        ...
        if delivery_mode == "terminal_goto" and not old_successors:
            ...
            if cut_instructions:
                ...
                if (
                    semantic_edge_source is not None
                    and semantic_edge_source is not endpoint
                ):
                    if (
                ...
                if len(
                    cut_instructions
                ) != 1 or not modifier.lower_proven_indirect_transfer_to_goto_now(
                    endpoint,
                    lowering_target,
        ...
        if not current_successors:
            ...
            return None
        if expected:
            direct_applied = int(modifier.apply(defer_post_apply_maintenance=True))
    ...
    for mutation in batch.conditional:
        ...
        if mutation.preserve_live_predicate:
            ...
            modifier.queue_lower_conditional_state_transition(
                ...
                proof_id=(f"resolver_live_entry_bridge:0x{int(port.predicate_ea):X}"),
                description=(
        ...
        if mutation.materialize_logical_source:
            ...
            if (
                ...
                return None
            predicate_vd_stkoff = int(mba.stkoff_ida2vd(int(port.predicate_ida_stkoff)))
            if port.predicate_stack_value is not None:
                ...
                condition_true = taken if int(port.condition_code) == 5 else fallthrough
                condition_false = (
            ...
            applied_count = int(modifier.apply(defer_post_apply_maintenance=True))
            try:
...
def _materialize_detached_snippet_templates(
    ...
    *,
                    ...
                    logger.info(
                        ...
                        [
                            (
                                ...
                                tuple(
                                    int(serial)
                                    for serial in candidate.successor_serials
                                ),
                                tuple(
                                    hex(int(ea))
    ...
    for template in selected:
        for block in template.blocks:
            created_serial = modifier.create_standalone_block(
                ...
                is_0_way=True,
                verify=False,
                stable_identity=_template_block_stable_identity(
                    block,
                    native_key=mutation_gateway.identity_index.native_key,
                ),
            ...
            created[(int(template.target_ea), int(block.source_serial))] = created_block
    ...
    if any(block is None for block in root_blocks.values()):
        logger.info("detached snippet import abstained: reason=root_relocation_failed")
        return _empty_import_result_for_boundary_ports(
            selected,
    ...
    for template in selected:
        for block in template.blocks:
            ...
            if destination is None:
                ...
                continue
            anchor_eas = tuple(
                int(instruction.ea) for instruction in _instructions(destination)
            )
            owned_instruction_eas = tuple(
                imported_ea
                for imported_ea in anchor_eas
                if (int(identity), int(imported_ea)) in _IMPORTED_INSTRUCTION_ORIGINS
            )
