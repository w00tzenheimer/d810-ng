...
class DeferredGraphModifier:
    ...
    def _mod_payload(self, mod: QueuedModification, mod_index: int | None = None) -> dict:
    ...
    def queue_goto_change(
        ...
        self.modifications.append(QueuedModification(
    ...
    def _realize_semantic_patch_plan(self, patch_plan, prepared_fragment):
        """Execute ordinary typed semantic PatchSteps under this gateway."""
    ...
    def _snapshot_semantic_fragment_inputs(self, plan: FragmentPlan):
        """Capture portable projection evidence before mutation begins."""
    ...
    def _publish_semantic_fragment_direct_fallthrough_root(
        ...
        if helper_binding.creation_ref is None:
            raise SemanticFragmentBackendRejected(
                "direct root helper lacks PlanBlockRef authority"
            )
        helper_serial = self._build_fallthrough_goto_helper(
            predecessor,
            replacement,
    ...
    def _publish_semantic_fragment_fallthrough_root(
        ...
        if helper_binding.creation_ref is None:
            raise SemanticFragmentBackendRejected(
                "conditional root helper lacks PlanBlockRef authority"
            )
        helper_serial = self._build_fallthrough_goto_helper(
            predecessor,
            replacement,
    ...
    def _publish_semantic_fragment_call_root_group(
        ...
        if helper_binding.creation_ref is None:
            raise SemanticFragmentBackendRejected(
                "call root helper lacks PlanBlockRef authority"
            )
        helper_serial = self._build_fallthrough_goto_helper(
            predecessor,
            replacement,
    ...
    def _publish_semantic_patch_roots(self, patch_plan, rollback_token) -> None:
        ...
        inventory = patch_plan.semantic_contract.root_inventory
        token = self._semantic_fragment_publication_token(plan, rollback_token)
    ...
    def _observe_published_semantic_fragment(self, plan: FragmentPlan):
        """Compatibility observation view; transactions use the graph view."""
        return self._observe_published_semantic_fragment_graph(plan).semantics
                ...
                for mod_type in unique_types:
                    ...
                    if len(same_type_mods) > 1:
                        if mod_type == ModificationType.BLOCK_TARGET_CHANGE:
                            ...
                            target_groups: dict[int | None, list[QueuedModification]] = {}
    ...
    def _apply_duplicate_block_and_redirect(
        ...
        *,
        ...
        try:
            ...
            if expected_serial is not None:
                self._bind_patch_block_creation(expected_serial, duplicated_blk.serial)
    ...
    def _apply_edge_split_trampoline(
        ...
        try:
            ...
            if expected_serial is not None and new_blk.serial != expected_serial:
                logger.info(
