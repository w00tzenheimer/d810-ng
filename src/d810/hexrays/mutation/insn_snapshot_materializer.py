...
def insn_snapshots_from_captured_body(
    ...
    if body.backend_id != HEXRAYS_INSN_SNAPSHOT_BODY_BACKEND_ID:
        raise TypeError(f"Unsupported captured body backend: {body.backend_id!r}")
...
def _iter_insn_snapshot_operands(
...
def _rebuild_mop_from_snapshot_operand(
    operand: object, safe_ea: int
) -> "ida_hexrays.mop_t":
    if isinstance(operand, MopSnapshot):
        ...
        raise TypeError(
            f"Unsupported InsertBlock operand type: {type(operand).__name__}"
        )
...
def _build_minsn_from_snapshot(
    ...
    for slot_name, operand in _iter_insn_snapshot_operands(insn_snapshot):
        setattr(
            new_ins, slot_name, _rebuild_mop_from_snapshot_operand(operand, safe_ea)
        )
    ...
    for ins in insns:
        ...
        try:
            if int(ins.opcode) == int(ida_hexrays.m_stx) and ins.d is not None:
                ...
                if d_kind == mop_d_t and ins.d.d is not None:
                    ...
                    l_kind = int(ins.d.d.l.t) if ins.d.d.l is not None else -1
                    r_kind = int(ins.d.d.r.t) if ins.d.d.r is not None else -1
                    logger.info(
        ...
        try:
            ...
            if ins.d is None or int(ins.d.t) != mop_S_t:
                continue
            l_is_const = ins.l is not None and int(ins.l.t) == mop_n_t
            r_is_const = ins.r is not None and int(ins.r.t) == mop_n_t
...
def materialize_insn_snapshots(
    ...
    *,
    ...
    insns = [
        _build_minsn_from_snapshot(instruction, safe_ea) for instruction in instructions
    ]
