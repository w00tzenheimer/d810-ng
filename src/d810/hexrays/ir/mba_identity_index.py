...
class PlanBlockReservation:
    ...
    def __post_init__(self) -> None:
        ...
        if not str(self.session_id) or int(self.generation) < 0:
            raise ValueError("reservation requires a session and generation")
...
class MbaBlockIdentityIndex:
    ...
    def generation_poisoned(self) -> bool:
        """Whether this exact live-MBA index was invalidated after an SDK write."""
        return self._poisoned_generation == self.generation

    @property
    def poisoned_failure(self) -> CfgTransactionFailure | None:
        """Return the first failure that poisoned this shared MBA generation."""
    ...
    def poison_generation(self, failure: CfgTransactionFailure) -> None:
        """Latch poison on the shared index for every gateway over this MBA."""
    ...
    def require_generation_usable(self) -> None:
        """Reject every operation routed through an invalid live MBA index."""
        ...
        *,
        ...
        if provenance is None:
            provenance = (
                ...
                else BlockHandleProvenance.CREATED_SYNTHETIC
            )
        ...
        if identity is None:
            if provenance is BlockHandleProvenance.OBSERVED_EPHEMERAL:
                handle = MbaBlockHandle.observed_ephemeral(
                    session_id=self.session_id,
                    token=token,
                )
            elif provenance is BlockHandleProvenance.CREATED_SYNTHETIC:
    ...
    def create_synthetic_handle(self) -> MbaBlockHandle:
        """Allocate an anonymous ephemeral handle for legacy SDK work.

        CREATED_SYNTHETIC authority is allocated only by reserve_plan_block().
        """
        self.require_generation_usable()
        return self.create_observed_ephemeral_handle()
    ...
    def create_observed_ephemeral_handle(self) -> MbaBlockHandle:
        """Allocate an unowned handle for a block discovered after SDK work."""
        self.require_generation_usable()
        return self._new_handle(
            None,
            provenance=BlockHandleProvenance.OBSERVED_EPHEMERAL,
    ...
    def begin_transaction(
        ...
        transaction_id = (
            attempt_id.attempt_id if attempt_id is not None else str(transaction_id)
        )
        ...
        if attempt_id is not None:
            self._attempt_by_transaction[transaction_id] = attempt_id
    ...
    def _validate_plan_attempt(
        ...
        if attempt_id.plan_id != plan_ref.plan_id:
            raise ValueError("plan reservation plan authority differs")
        transaction_id = attempt_id.attempt_id
        ...
        active_attempt = self._attempt_by_transaction.get(transaction_id)
    ...
    def reserve_plan_block(
        ...
        *,
        ...
        """Allocate plan ownership and a logical version before SDK insertion."""
        self.require_generation_usable()
        transaction_id = self._validate_plan_attempt(attempt_id, plan_ref)
        key = (attempt_id, plan_ref)
    ...
    def bind_reserved_plan_block(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("plan reservation belongs to another session or generation")
    ...
    def commit_proxy_transaction(
        ...
        for proxy_token, proxy in self._proxies_by_token.items():
            ...
            if serial is None:
                raise ValueError(
                    "future published logical block has no live serial: "
                    f"proxy={proxy_token} handle={future.handle.token} "
                    f"action={actions.get(proxy_token, 'published')}"
                )
    ...
    def record_insert(
        ...
        *,
        ...
        """Record non-owned SDK insertion without consuming plan authority."""
        self.require_generation_usable()
    ...
    def _record_insert(
        ...
        *,
        ...
        """Bind one insertion; owned creation is reserved for the typed caller."""
        transaction_id = str(transaction_id)
        serials = self._serials_by_transaction.get(transaction_id)
        if serials is None:
    ...
    def record_clone(
        ...
        *,
        ...
        if created.provenance is BlockHandleProvenance.CREATED_SYNTHETIC:
            raise ValueError("synthetic clone requires PlanBlockRef authority")
        self.stage_new_proxy(
            transaction_id=str(transaction_id),
            handle=created,
            returned_serial=int(returned_serial),
