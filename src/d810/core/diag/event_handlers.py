...
def _handle_z3_predicate_proof(ev: Z3PredicateProofObserved) -> None:
    ...
    persist_lifecycle_event(
        ...
        LifecycleEventObserved(
            ...
            summary=f"{ev.transform_id} {ev.operation}: {ev.status.value}",
            payload={
                ...
                "reason": None if ev.reason is None else ev.reason.value,
            },
            timestamp=ev.timestamp,
        ),
        snapshot_id=None,
...
def _handle_unflat_candidate_outcome(ev: UnflattenCandidateOutcomeObserved) -> None:
    try:
        conn = get_diag_conn(int(ev.func_ea))
    except Exception:
        return
