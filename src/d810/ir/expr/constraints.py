...
class EqualityConstraint(ConstraintExpr):
    ...
    def _check_bnot_constraint(self, candidate: dict[str, Any]) -> bool:
        ...
        # Structural BNOT comparison via the injected backend oracle (Hex-Rays-backed);
        # None in portable use, where this matching-time check is never invoked.
        mop_ops = get_mop_ops()
        if mop_ops is None:
            return False
        return mop_ops.equal_bnot_mop(left_mop, right_mop)
