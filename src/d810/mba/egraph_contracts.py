...
def _freeze_json_value(value: object) -> JsonValue:
    ...
    if isinstance(value, Mapping):
        ...
        frozen = {key: _freeze_json_value(item) for key, item in sorted(value.items())}
...
def _cost(
    name: str, value: object, *, json_input: bool = False
) -> tuple[int, int] | None:
    if value is None:
        return None
...
def _trace(name: str, value: object) -> tuple[tuple[str, str, tuple[str, ...]], ...]:
    ...
    for row in value:
        if type(row) is not tuple or len(row) != 3:
            raise ValueError(f"{name} entries must be (family, source, aliases) tuples")
...
def _json_trace(
    ...
    for row in rows:
        if type(row) is not list or len(row) != 3:
            raise ValueError(f"{name} entries must be [family, source, aliases] lists")
...
class MbaEgraphOptions:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.saturation_rounds) is not int
            or not 1 <= self.saturation_rounds <= 6
        ):
            raise ValueError("saturation_rounds must be an integer from 1 to 6")
        if self.function_time_budget_ms is not None:
            if (
                type(self.function_time_budget_ms) is not int
                or not 0 <= self.function_time_budget_ms <= 5_000
            ):
                raise ValueError(
        ...
        if (
            type(self.learned_replay_max_entries) is not int
            or not 1 <= self.learned_replay_max_entries <= MAX_LEARNED_REPLAY_ENTRIES
        ):
            raise ValueError(
                "learned_replay_max_entries must be an integer from 1 to 4096"
            )
        if (
            type(self.learned_replay_max_bytes) is not int
            or not 1 <= self.learned_replay_max_bytes <= MAX_LEARNED_REPLAY_BYTES
        ):
            raise ValueError(
                "learned_replay_max_bytes must be an integer from 1 to 16777216"
            )
        ...
        unsupported = tuple(
            family for family in families if family not in EGRAPH_FAMILIES
        )
        if unsupported:
            raise ValueError(
        ...
        if not maturities or any(
            value not in {member.name for member in IRMaturity} for value in maturities
        ):
    ...
    def from_dict(cls, payload: object) -> "MbaEgraphOptions":
        ...
        if "maturities" in values:
            values["maturities"] = tuple(
                _json_tuple("maturities", values["maturities"])
            )
        try:
            return cls(**values)
...
class EgraphExtractionReceipt:
    ...
    def __post_init__(self) -> None:
        if (
            self.native_matcher_elapsed_ms is not None
            and type(self.native_matcher_elapsed_ms) is not float
        ):
            raise ValueError(
        ...
        for name in (
            ...
            object.__setattr__(
                self, name, _optional_nonnegative_int(name, getattr(self, name))
            )
        object.__setattr__(
            self, "rule_firings", _nonnegative_int("rule_firings", self.rule_firings)
        )
        object.__setattr__(
            self,
            "egraph_work_units",
            _nonnegative_int("egraph_work_units", self.egraph_work_units),
        )
        for name in (
            "elapsed_ms",
            ...
            object.__setattr__(
                self,
                name,
                _timing(name, getattr(self, name), optional=name != "elapsed_ms"),
            )
        object.__setattr__(self, "input_cost", _cost("input_cost", self.input_cost))
        object.__setattr__(
            self, "extracted_cost", _cost("extracted_cost", self.extracted_cost)
        )
        object.__setattr__(
            self,
            "canonical_input_cost",
            _cost("canonical_input_cost", self.canonical_input_cost),
        )
        object.__setattr__(
            self,
            "selected_aliases",
            _string_tuple("selected_aliases", self.selected_aliases),
        )
        object.__setattr__(
            self,
            "normalization_steps",
            _string_tuple("normalization_steps", self.normalization_steps),
        )
        object.__setattr__(
            self, "blockers", tuple(sorted(_string_tuple("blockers", self.blockers)))
        )
        object.__setattr__(
            self, "derivation_trace", _trace("derivation_trace", self.derivation_trace)
        )
        object.__setattr__(
            self, "replayed_trace", _trace("replayed_trace", self.replayed_trace)
        )
        object.__setattr__(
            self, "native_profile", self._normalize_profile(self.native_profile)
        )
        if self.skip_reason is not None and not isinstance(
            self.skip_reason, EgraphSkipReason
        ):
        ...
        if self.cache_status is not None and (
            ...
            or self.cache_status
            not in {"disabled", "miss", "hit", "stale", "malformed", "evicted"}
        ):
            raise ValueError("unknown e-graph cache status")
        object.__setattr__(
            self,
            "selected_family",
            _optional_string("selected_family", self.selected_family),
        )
        object.__setattr__(
            self,
            "selected_source",
            _optional_string("selected_source", self.selected_source),
        )
        object.__setattr__(
            self, "island_class", _optional_string("island_class", self.island_class)
        )
        object.__setattr__(
            self,
            "island_fingerprint",
            _optional_string("island_fingerprint", self.island_fingerprint),
        )
        object.__setattr__(
            self,
            "template_source_name",
            _optional_string("template_source_name", self.template_source_name),
        )
        object.__setattr__(
            self,
            "template_fallback_reason",
            _optional_string("template_fallback_reason", self.template_fallback_reason),
        )
        object.__setattr__(
            self,
            "replay_fallback_reason",
            _optional_string("replay_fallback_reason", self.replay_fallback_reason),
        )
        object.__setattr__(
            self, "cache_key", _optional_string("cache_key", self.cache_key)
        )
        if self.native_matcher_backend is not None and (
            type(self.native_matcher_backend) is not str
            or self.native_matcher_backend not in {"python", "cython", "structural"}
        ):
        ...
        object.__setattr__(
            self,
            "backend_version",
            _optional_string("backend_version", self.backend_version),
        )
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            "input_cost": list(self.input_cost)
            if self.input_cost is not None
            else None,
            "extracted_cost": list(self.extracted_cost)
            ...
            "derivation_trace": [
                [family, source, list(aliases)]
                for family, source, aliases in self.derivation_trace
            ],
            ...
            "native_profile": None
            if self.native_profile is None
            else _json_ready(self.native_profile),
            ...
            "canonical_input_cost": list(self.canonical_input_cost)
            ...
            "replayed_trace": [
                [family, source, list(aliases)]
                for family, source, aliases in self.replayed_trace
            ],
    ...
    def from_dict(cls, payload: object) -> "EgraphExtractionReceipt":
        ...
        try:
            return cls(
                ...
                extracted_cost=_cost(
                    "extracted_cost", payload["extracted_cost"], json_input=True
                ),
                ...
                selected_aliases=_json_tuple(
                    "selected_aliases", payload["selected_aliases"]
                ),
                derivation_trace=_json_trace(
                    "derivation_trace", payload["derivation_trace"]
                ),
                ...
                skip_reason=(
                    None
                    if payload["skip_reason"] is None
                    else EgraphSkipReason(payload["skip_reason"])
                ),
                canonicalizer_version=payload["canonicalizer_version"],
                canonical_input_cost=_cost(
                    "canonical_input_cost",
                    payload["canonical_input_cost"],
                    json_input=True,
                ),
                normalization_steps=_json_tuple(
                    "normalization_steps", payload["normalization_steps"]
                ),
