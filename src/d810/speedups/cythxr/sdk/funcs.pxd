...
cdef extern from "funcs.hpp":
    ...
    ctypedef int diffpos_t  # A position in the difference source. This is an abstract...
    ...
    cdef cppclass rangeset_t:  # An ordered set of non-overlapping address ranges
        ...
        const_iterator lower_bound(int ea) const  # Get the first range that contains at least one ea_
        const_iterator upper_bound(int ea) const  # Get the first range such that every ea_t value in 
    ...
    bint test_bit(uchar* bitmap, int bit)  # ---------------------------------------------------------...
    ...
    uval_t rotate_left(uval_t x, int count, int bits, int offset)  # Rotate left - can be used to rotate a value to the right...
    ...
    range_t inf_get_privrange()  #  </INF _ACCESSORS>
