...
- **L6 jump-table / m_jtbl** — `_jtbl_routing` reads `mcases_t` (`tail.r.c`) → `{case:handler}` directly; `_build_jtbl_unflatten_plan` drains the switch to a linear chain. `old_target` is PER-WRITER (entry→jtbl, handlers→re-dispatch join). Density+count (not magnitude) force the table; deobfuscation DRAINS a switch, never inserts one. Obs: `flat_jtbl_insert_unflatten.json` (llr-6aiq).
- **L8 conditional synthesis (P2b)** — branchless `(token&1)?K1:K2` survives as one `m_or`; `_find_low_bit_predicate` recovers `(token&1)` live from the tree, `queue_lower_conditional_state_transition` lowers H0 to `m_jnz cond,0` 2-way. Renders a synthesized `if/else`, no jcc to preserve (complement of P2a). L12 MBA machinery not needed. Obs: `flat_branchless_synth.json` (llr-se40).
- **L7 multi-block region** — 2-block shared region (head + internal `if(token&2)` branch + tail). BOTH reconstructions: (1) join-preserve (general unflatten → region head a shared JOIN, `-0x22` once) and (2) de-share/duplicate (2-pass: clone conditional HEAD per path via `create_conditional_redirect`, then de-share the multi-pred TAIL P3-style; tail re-found by instruction-EA overlap across the serial shift). Obs: `flat_region_multiblock.json` (llr-5866).
...
| L6 | jump-table / N-way dispatch | `NormalizeNWayDispatcherExit`,`CanonicalizeJumpTableCaseOverlap`,`dispatcher_materialization` | ~~`m_jtbl` fixture + lab-validate~~ **DONE** — `mcases_t`-reading extractor drains the switch to a linear chain | MED | done |
| L7 | multi-block subgraph insert | `DuplicateReplayAndRedirect`,`new_blocks: PatchBlockSpec` | ~~insert a captured *region* (2+ blocks, internal edges)~~ **DONE** — join-preserve + 2-pass de-share (clone head + de-share tail) | MED | done |
| L8 | conditional synthesis (P2b) | `LowerConditionalStateTransition`,`CreateConditionalRedirect` | ~~branchless/cmov → synthesize the branch~~ **DONE** — recover `(token&1)` from the m_or, lower to `m_jnz` 2-way | MED | done |
