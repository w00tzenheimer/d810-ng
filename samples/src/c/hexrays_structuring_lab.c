...
/*
 * OLLVM-realistic terminal-tail shared convergence (simplified from _hodur_func).
 *
 * Same "D810-like bad shape" as hexrays_lab_terminal_tail_shared_convergence
 * (N terminals route through one shared guard -> one shared `return result`), but
 * with the properties d810's OLLVM-tuned detection actually engages -- the two
 * gaps the tiny-state fixture exposed:
 ...
 *   - the dispatcher's compares are plain `state == K` (mop_S) so _detect_stkoff
 *     finds the state var (no MBA mop_d at the entry -- the cursor return-check
 *     lives inside each handler instead).
