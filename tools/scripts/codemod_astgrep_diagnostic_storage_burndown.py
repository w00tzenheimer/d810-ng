...
def build_manifest(
    root: Path,
    inventories: tuple[RuleInventory, ...],
) -> dict[str, Any]:
    return {
        "generated_by": (
        ...
        "moved_imports": {
            ...
            "symbol_renames": [
                ...
                for (module, name), to_module in sorted(MOVED_SYMBOL_RENAMES.items())
            ],
        },
...
def render_report(inventories: tuple[RuleInventory, ...]) -> str:
    lines = [
        ...
        "",
        "Generated by `tools/scripts/codemod_astgrep_diagnostic_storage_burndown.py`.",
        "",
        ...
        if isinstance(node, ast.Import):
            ...
            if end_lineno != lineno:
                has_moved_symbol = any(
                    alias.name != "*" and (module, alias.name) in MOVED_SYMBOL_RENAMES
                    for alias in node.names
                )
            ...
            if target is not None:
                new_line = new_line.replace(
                    f"from {module} import", f"from {target} import", 1
                )
            else:
                ...
                if star_import and moved:
                    warnings.append(f"{path}:{lineno}: manual review for star import")
                    continue
                if moved:
