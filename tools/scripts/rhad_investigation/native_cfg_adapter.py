...
def needs_native_flow_decode(fact: NativeFlowBlockFact | None) -> bool:
    ...
    if fact is None or int(fact.end_ea) <= int(fact.start_ea):
        return True
    if fact.force_stop or fact.is_indirect_jump_tail or fact.is_return_tail:
        return False
...
def can_decode_proven_native_successor(
    *,
    ...
    return bool(
        is_code
        and (
            owner_func_ea is None
            or int(owner_func_ea) == int(requested_func_ea)
        )
    )
...
def is_native_direct_control_operand(
    *,
    ...
    return bool(
        operand_is_near
        and (is_call or is_basic_block_end or has_stop_feature)
    )
...
def _outgoing_edges(fact: NativeFlowBlockFact) -> tuple[NativeEdge, ...]:
    ...
    if len(successors) > 2:
        raise ValueError(
            "native flow fact at "
            f"0x{int(fact.start_ea):X} has more than two successors"
        )
    if len(successors) == 2:
        ...
        return (
            NativeEdge(NativeEdgeKind.CONDITIONAL_TRUE, int(branch_target)),
            NativeEdge(NativeEdgeKind.CONDITIONAL_FALSE, false_target),
        )
    if len(successors) == 1:
        kind = (
        ...
        return (NativeEdge(kind, successors[0]),)
    return ()
