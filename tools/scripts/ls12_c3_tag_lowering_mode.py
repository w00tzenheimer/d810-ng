...
EDITS: tuple[tuple[str, str, list[tuple[str, str]]], ...] = (
    (
        "strategies/topological_sort.py",
    ...
    (
        "strategies/exact_conditional_node.py",
        "from d810.core import logging",
        [
            (
    ...
    (
        "strategies/exact_conditional_alias.py",
        "from d810.core import logging",
        [
            (
    ...
    (
        "strategies/exact_conditional_fork.py",
        "from d810.core import logging",
        [
            (
    ...
    (
        "prototypes/exact_conditional_bridge.py",
        "from d810.core import logging",
        [
            (
    ...
    (
        "strategies/semantic_exact_node.py",
        "from d810.core import logging",
        [
            (
    ...
    for rel, import_anchor, attrs in EDITS:
        ...
        if IMPORT not in text:
            if text.count(import_anchor) != 1:
                print(f"  FAIL {rel}: import anchor count != 1")
                ok = False
                continue
        ...
        for old, new in attrs:
            if new in text:
                print(f"  SKIP {rel}: attr already present")
                continue
            if text.count(old) != 1:
                print(f"  FAIL {rel}: attr anchor count={text.count(old)} != 1")
                ok = False
                continue
