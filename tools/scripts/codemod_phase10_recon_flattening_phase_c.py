...
class ModuleRenameTransformer(cst.CSTTransformer):
    def leave_ImportFrom(
        ...
        if module_code in PHASE_C_RENAMES:
            return updated_node.with_changes(
                module=cst.parse_expression(PHASE_C_RENAMES[module_code])
            )
        return updated_node
    ...
    def leave_ImportAlias(
        ...
        if name_code in PHASE_C_RENAMES:
            return updated_node.with_changes(
                name=cst.parse_expression(PHASE_C_RENAMES[name_code])
            )
        return updated_node
...
def _rewrite_text(text: str) -> str:
    ...
    for old, new in PHASE_C_RENAMES.items():
...
def main() -> int:
    ...
    for path in _iter_python_files(root):
        ...
        if not any(old in src for old in PHASE_C_RENAMES):
            continue
