...
def main() -> int:
    ...
    movable = [
        r
        for r in rows
        if r.get("role") in RELOC and (WT / "src/d810" / r["path"]).exists()
    ]
    ...
    print(
        f"  same-layer SCCs (atomically movable): {len(same_layer_sccs)} ({sum(len(c) for c in same_layer_sccs)} files)"
    )
    print(
        f"  CROSS-LAYER SCCs (IRREDUCIBLE w/o refactor): {len(cross_layer_sccs)} ({trapped} files)"
    )
        ...
        units.append(
            {
                "files": [
                    {
                        "path": p,
                        "old_module": old_module(p),
                        "dest": by_path[p]["destination_module"],
                        "role": by_path[p]["role"],
                        "seam": by_path[p].get("needs_seam", False),
                    }
                    for p in comp
                ],
                "atomic": len(comp) > 1,
                ...
                "gate_class": "golden"
                if any(by_path[p]["role"] in ("evidence", "mutation") for p in comp)
                else "static",
            }
        )
