...
def run_gate(
    gate_class: str, *, worktree: str, e2e_selection: list[str] | None = None
) -> GateResult:
...
def _run_static(worktree: str) -> GateResult:
    ...
    for name, cmd in STATIC_CHECKS:
        ...
        if cp.returncode != 0 or broken:
            return GateResult(
                False, failed_check=name, detail=captured[name], captured=captured
            )
...
def _run_golden(worktree: str, selection: list[str]) -> GateResult:
    ...
    if cp.returncode != 0:
        return GateResult(
            False, failed_check="golden-e2e", detail=captured["e2e"], captured=captured
        )
    return GateResult(True, captured=captured)
