...
class EditSimulatorTransformer(cst.CSTTransformer):
    ...
    def leave_FunctionDef(
        ...
        if (
            updated_node.name.value == "project_post_state"
            and not self.added_diagnostic
        ):
                ...
                if isinstance(stmt, cst.SimpleStatementLine):
                    for expr in stmt.body:
                        if isinstance(expr, cst.Assign):
                            if (
                                isinstance(expr.targets[0].target, cst.Name)
                                and expr.targets[0].target.value == "simulated"
                            ):
...
def add_projected_cfg_dump(src: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            in_project_post_state
            and not added
            and "simulated = simulate_edits(" in line
        ):
            # Get the indentation
            indent = len(line) - len(line.lstrip())
            new_lines.append("\n")
            new_lines.append(
                " " * indent + "# DIAGNOSTIC: Projected CFG adjacency dump\n"
            )
...
def add_reachability_audit(src: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            ...
            or (in_apply_reorder and line.startswith("class "))
        ):
            in_apply_reorder = False
            found_mark_chains = False
        ...
        if in_apply_reorder and not added and "mba.mark_chains_dirty()" in line:
            ...
            new_lines.append(
                indent
                + '        logger.debug("REACHABILITY: %d unreachable blocks: %s", len(unreachable), sorted(unreachable))\n'
            )
            ...
            new_lines.append(
                indent
                + '            logger.debug("  block %d type=%s preds=%s succs=%s", s, blk.type, preds, succs)\n'
            )
            new_lines.append(indent + "    else:\n")
            new_lines.append(
                indent
                + '        logger.debug("REACHABILITY: all %d blocks reachable", mba.qty)\n'
            )
            new_lines.append(indent + "    \n")
...
def add_reorder_debug_logging(src: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            in_apply_reorder
            and not added
            and 'logger.info("reorder_blocks: skipped' in line
        ):
            ...
            new_lines.append(indent + "if logger.debug_on:\n")
            new_lines.append(
                indent + '    logger.debug("dfs_block_order: %s", dfs_block_order)\n'
            )
            new_lines.append(
                indent + '    logger.debug("runtime_non_2way: %s", runtime_non_2way)\n'
            )
            new_lines.append(
                indent
                + '    logger.debug("runtime_old_to_new (initial): %s", old_to_new)\n'
            )
            added = True
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Add diagnostic infrastructure for TopologicalSort regression"
    )
