#!/usr/bin/env python3
"""Operator CLI for D810 dump and diagnostics workflows.
...
# Detect whether d810cli is being run from inside an existing worktree
# checkout (under ``<repo>/.worktrees/<name>/tools/d810cli.py``) or from a
# bare repo checkout (``<repo>/tools/d810cli.py``).  When inside a worktree
...
def cmd_pseudocode_capture(args: argparse.Namespace) -> int:
    ...
    With ``--dump`` this is a pure parser/DB insertion path. Without it, the
    command first runs the existing Docker dump workflow, then parses the dump
    it just produced.
    """
    _ensure_src_on_path()
    ...
    if args.dump:
        ...
        if not dump_path.is_file():
            _die(f"dump not found: {dump_path}")
    else:
        rc = cmd_dump(
            argparse.Namespace(
                ...
                prefix=args.prefix,
                label=args.label,
                capture_post_maturity=args.capture_post_maturity,
                no_debug_logging=args.no_debug_logging,
                extra=args.extra,
            )
        )
        if rc != 0:
            return rc
        dump_path = latest_dump(args.worktree)
    ...
    print(render_capture_summary(row))
    print(f"DB={db_path}")
    print(f"DUMP={dump_path}")
    return 0
...
def cmd_snap_render(args: argparse.Namespace) -> int:
    ...
    resolution + subprocess shell-out, per the d810cli architecture
...
def cmd_trace(args: argparse.Namespace) -> int:
    ...
    if rc != 0:
        print(
            f"d810cli: trace: dump returned {rc}; skipping diag report",
            file=sys.stderr,
        )
        return rc
    wt = args.worktree
    ...
    print(
        f"d810cli: trace: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_residual_worksheet(args: argparse.Namespace) -> int:
    ...
    Distinct from `d810cli.py residuals`, which is a text grep over
    ...
    print(
        f"d810cli: residual-worksheet: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_egress_plan(args: argparse.Namespace) -> int:
    ...
    print(
        f"d810cli: egress-plan: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_frontier_diagnostics(args: argparse.Namespace) -> int:
    ...
    print(f"DB={db}", file=sys.stderr)
    print(
        f"d810cli: frontier-diagnostics: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_returns(args: argparse.Namespace) -> int:
    ...
    print(
        f"d810cli: returns: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_reconcile(args: argparse.Namespace) -> int:
    ...
    the worktree's d810.log so a fresh `d810cli dump` followed by
    `d810cli reconcile` Just Works.
    """
    wt = args.worktree
    worktree = worktree_dir(wt)
    db = resolve_db(wt, args.db)
    ...
    print(f"LOG={log_path}", file=sys.stderr)
    print(
        f"d810cli: reconcile: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_gates(args: argparse.Namespace) -> int:
    ...
    print(
        f"d810cli: gates: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_byte_audit(args: argparse.Namespace) -> int:
    ...
    print(f"DB={db}", file=sys.stderr)
    print(
        f"d810cli: byte-audit: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_compose_evidence_explain(args: argparse.Namespace) -> int:
    ...
    print(
        f"d810cli: compose-evidence-explain: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_admission_explain(args: argparse.Namespace) -> int:
    ...
    print(f"LOG={log_file}", file=sys.stderr)
    print(
        f"d810cli: admission-explain: diag argv: {' '.join(diag_argv)}",
        file=sys.stderr,
    )
    return subprocess.call(diag_argv, env=env)
...
def cmd_inspect(args: argparse.Namespace) -> int:
    ...
    Ports ``tools/scripts/inspect_hodur_dump.sh`` into d810cli; runs the
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="d810cli",
        description="Operator CLI for D810 dump and diagnostics workflows.",
    )
    ...
    pseudocode = sub.add_parser(
        "pseudocode",
        help="pseudocode capture and inspection workflows",
    )
    pseudocode_sub = pseudocode.add_subparsers(dest="pseudocode_cmd", required=True)

    sp = pseudocode_sub.add_parser(
        "capture",
        help="capture before/after pseudocode into a SQLite DB",
    )
    _add_worktree(sp)
    sp.add_argument("-f", "--function", required=True)
    sp.add_argument("-p", "--project", default="example_libobfuscated.json")
    sp.add_argument(
        "--db",
        default=None,
        help="capture DB path (default: D810_CAPTURE_DB or repo .d810_capture.db)",
    )
    sp.add_argument(
        "--dump",
        help="parse an existing dump file instead of running the Docker dump workflow",
    )
    sp.add_argument("--binary-name", default=None)
    sp.add_argument(
        "--prefix",
        default="pseudocode_capture",
        help="dump filename prefix when running Docker",
    )
    sp.add_argument(
        "--label",
        default="capture",
        help="dump filename label when running Docker",
    )
    sp.add_argument(
    ...
    sp.add_argument(
        "--extra",
        action="append",
        help="extra pytest arg for Docker dump (repeatable); if any provided, replaces defaults",
    )
    ...
    sp = sub.add_parser(
        ...
        help=(
            ...
            " residual-worksheet`. Distinct from `d810cli.py residuals`,"
